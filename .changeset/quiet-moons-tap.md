...
`@sl-design-system/toggle-button` has a major release in which the `[icon-only]`, `[text-only]`,
and `[error]` attributes were replaced by CSS custom states. The `[pressed]` attribute is unchanged,
so the group still targets `::slotted(sl-toggle-button[pressed])`. The border radius of the first
and last button is also applied through the `--_button-border-radius` custom property rather than
separate corner properties.
