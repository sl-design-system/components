...
- Child buttons and menu-buttons inherit `fill` and `inverted` attributes from the toolbar
- Buttons and menu-buttons inside the toolbar can now have different variants and fills mixed together

**Backwards compatibility:** Buttons and menu-buttons used in toolbar that already have an explicit `fill` or `variant` attribute set will keep their own value — the toolbar only propagates these attributes to elements that don't have them set explicitly.
