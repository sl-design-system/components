...
- `/packages/components/shared` - Shared utilities, mixins, and controllers
...
- `src/` - Source directory containing all implementation files

`package.json` is the only file in the root of the package; everything else lives in `src/`. Components do **not** have their own TypeScript, ESLint or stylelint configuration — `packages/components/tsconfig.json`, `packages/components/eslint.config.mjs` and `packages/components/stylelint.config.mjs` cover every component in the monorepo.

**Important:** A new component package needs no registration anywhere. The root `package.json` workspaces include `packages/components/*` and `packages/components/tsdown.config.mjs` builds every workspace under it, so a `yarn install` is all that is needed.

### Source Files in `src/`

- `index.ts` - Main entry point, exports the component class
- `register.ts` - Registers the component with the custom elements registry
- `<component>.ts` - Main component implementation
- `<component>.css` - Component styles (plain CSS, imported with `with { type: 'css' }`)
...
Both `exports` and `publishConfig.exports` are **generated by tsdown** during the build — copy them from an existing component when scaffolding, then let `yarn build` maintain them. The `@sl-design-system/source` export condition selects TypeScript sources for local development; the default points at built JavaScript in `dist`, including when npm publishes without applying `publishConfig.exports`.

If a component needs entry points beyond `src/index.ts` and `src/register.ts`, list all of them in a `tsdown` field in its `package.json` (see `shared`, `grid`, `format-date`):
...
1. **Use `.js` extensions** in imports of TypeScript modules (code runs in browser)
...
3. **Styles in plain CSS** - Place in `<component>.css` files
   - Import with the `with { type: 'css' }` import attribute: `import styles from './component.css' with { type: 'css' };`
   - This yields a `CSSStyleSheet` you assign directly to `static styles`
   - There is no build step and no `.scss`/`.scss.ts`; the `@sl-design-system/rolldown-plugin-css-sheet` plugin handles the import in tsdown, Vitest and Storybook alike
...
The packages are built with [tsdown](https://tsdown.dev):

```bash
yarn build
```

To build the components only (add `--filter <package-name>` for a single package, `--watch` to rebuild on change):

```bash
yarn tsdown --config packages/components/tsdown.config.mjs
```

You do not need to build while developing — Storybook and the tests consume the TypeScript sources directly.

### Run Tests

Tests run in a real browser through Vitest + Playwright. Pass a path to scope the run to one package; drop `--run` for watch mode:

```bash
yarn vitest --project=unit --run packages/components/<component>
```
...
stylelint --config packages/components/stylelint.config.mjs 'packages/components/<component>/**/*.css'
```

Formatting is done with `oxfmt` (not prettier), which runs on save in vscode and via `lint-staged` on commit.
