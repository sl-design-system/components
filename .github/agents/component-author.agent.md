...
Both `exports` and `publishConfig.exports` are **generated by tsdown** during the build — copy them from an existing component when scaffolding, then let `yarn build` maintain them. During development `exports` points at the TypeScript sources in `src`; `publishConfig.exports` points at the built `dist` output used when publishing.
