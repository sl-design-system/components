...
- [`combobox`](https://github.com/sl-design-system/components/blob/main/packages/components/combobox/CHANGELOG.md) adds a new `shape` property (defaulting to `rect`, also accepts `pill`), and an `option-disabled-path` to map disabled option state when rendering from the options property. It automatically exports listbox components when importing `register.js`.
...
- [`tooltip`](https://github.com/sl-design-system/components/blob/main/packages/components/tooltip/CHANGELOG.md) is completely rewritten with a simpler, native API using popover and CSS Anchor Positioning (note: you may need the CSS Anchor Positioning polyfill for full browser support).
...
- [`menu`](https://github.com/sl-design-system/components/blob/main/packages/components/menu/CHANGELOG.md) fixes nested submenu item clicks being intercepted by parent menu items, and fixes double-click menu reopening. Menu items now properly expose `aria-disabled="true"` when disabled.
