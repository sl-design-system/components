...
async function waitForControls(form, names) {
  for (
    let i = 0;
...
async function waitForFieldValue(field, value) {
...
describe('example-child-form', () => {
  it('should set child form values from the parent form value', async () => {
    ...
    const childForm = form.querySelector('example-child-form-test');
    await childForm.updateComplete;
    const addressForm = childForm.renderRoot.querySelector('sl-form');
    ...
    await new Promise(requestAnimationFrame);
    await addressForm.updateComplete;
    const parentTextField = name => form.querySelector(`sl-text-field[name="${name}"]`),
      childTextField = name => childForm.renderRoot.querySelector(`sl-text-field[name="${name}"]`);
...
//# sourceMappingURL=child-form.spec.js.map
