...
By default, validation messages are announced by the live-region announcer. If your form renders an aggregated error message (via `<sl-inline-message>`), set `announce-errors="false"` on `<sl-form>` to avoid duplicate screen reader announcements.
