...
describe('sl-month-view', () => {
  ...
  describe('indicator dates', () => {
    ...
    it('should render a tooltip for the indicator with label', async () => {
      ...
      await el.updateComplete;
      const button = el.renderRoot.querySelector('button[part~="indicator"]'),
    ...
    it('should only show one tooltip at a time and maintain ARIA stability', async () => {
      ...
      const button13 = el.renderRoot.querySelector(`td[data-date="${date13}"] button`);
      const button14 = el.renderRoot.querySelector(`td[data-date="${date14}"] button`);
  ...
  describe('selection', () => {
    ...
    describe('current month', () => {
      it('should select a day when clicked', () => {
        let date = null;
        el.addEventListener('sl-select', event => (date = event.detail));
        el.renderRoot
          .querySelector(
    ...
    describe('not the current month', () => {
      it('should select a day when clicked', async () => {
        let date = null;
        el.addEventListener('sl-select', event => (date = event.detail));
        el.renderRoot.querySelector('button[part~="next-month"]')?.click();
        await el.updateComplete;
      ...
      it('should emit an sl-change event when the month changes due to selection', () => {
        let date = null;
        el.addEventListener('sl-change', event => (date = event.detail));
        el.renderRoot.querySelector('button[part~="next-month"]')?.click();
        expect(date).to.equalDate(new Date(2023, 3, 1));
      });
    });
