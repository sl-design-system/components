...
describe('sl-month-view', () => {
  ...
  describe('Click', () => {
    ...
    it('Should not emit sl-select when readonly is set', async () => {
      ...
      expect(button).to.exist; // readonly still renders buttons (disabled)
  ...
  describe('Keyboard navigation (selection)', () => {
    ...
    it('Should not emit change when ArrowUp weekly step stays within month', async () => {
      const btnFifteen = Array.from(el.renderRoot.querySelectorAll('button')).find(
        b => b.textContent?.trim() === '15'
      ) as HTMLButtonElement;
      expect(btnFifteen).to.exist;
      const onChangeSpy = spy();
      el.addEventListener('sl-change', onChangeSpy);
      btnFifteen.dispatchEvent(new KeyboardEvent('keydown', { key: 'ArrowUp', bubbles: true }));
      await el.updateComplete;
      expect(onChangeSpy).not.to.have.been.called;
    });

    it('Should not emit change when ArrowDown weekly step stays within month', async () => {
      const btnTen = Array.from(el.renderRoot.querySelectorAll('button')).find(
        b => b.textContent?.trim() === '10'
      ) as HTMLButtonElement;
      expect(btnTen).to.exist;
      const onChangeSpy = spy();
      el.addEventListener('sl-change', onChangeSpy);
      btnTen.dispatchEvent(new KeyboardEvent('keydown', { key: 'ArrowDown', bubbles: true }));
      await el.updateComplete;
      expect(onChangeSpy).not.to.have.been.called;
    });
  });
  ...
  describe('Attribute Converters', () => {
    ...
    it('Should parse indicator attribute JSON into Date instances', async () => {
      ...
      el.setAttribute('indicator', JSON.stringify([{ date: iso, color: 'grey', label: 'Attr label' }]));
      await el.updateComplete;
  ...
  describe('Combined states', () => {
    ...
    it('Should apply both indicator color and negative part when date matches both', async () => {
      ...
      el.negative = [date];
      await el.updateComplete;
      const btn = Array.from(el.renderRoot.querySelectorAll('button')).find(b => b.textContent?.trim() === String(day));
      expect(btn).to.exist;
      expect(btn?.matches('[part~="indicator"]')).to.be.true;
      expect(btn?.matches('[part~="indicator-yellow"]')).to.be.true;
      expect(btn?.matches('[part~="negative"]')).to.be.true;
    });
  });
});
