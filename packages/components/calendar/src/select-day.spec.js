...
describe('sl-select-day', () => {
  ...
  describe('defaults', () => {
    ...
    it('should emit an sl-select event when a date is selected', async () => {
      ...
      el.renderRoot
        .querySelector('sl-month-view:not([inert])')
    ...
    it('should emit an sl-toggle "month" event when clicking current month button', async () => {
      ...
      el.renderRoot.querySelector('sl-button.current-month')?.click();
      await el.updateComplete;
    ...
    it('should emit an sl-toggle "year" event when clicking current year button', async () => {
      const onToggle = spy();
      el.addEventListener('sl-toggle', onToggle);
      el.renderRoot.querySelector('sl-button.current-year')?.click();
      await el.updateComplete;
      expect(onToggle).to.have.been.calledOnce;
    ...
    describe('days of the week', () => {
      it('should render the days of the week', () => {
        const daysOfWeek = Array.from(
          el.renderRoot.querySelectorAll('.days-of-week .day-of-week')
      ...
      it('should have role list with listitems for days of the week', () => {
        const daysOfWeek = el.renderRoot.querySelector('.days-of-week'),
          dayElements = Array.from(daysOfWeek?.querySelectorAll('.day-of-week') ?? []);
  ...
  describe('min/max', () => {
    ...
    it('should not change month when at the min boundary and using the keyboard', async () => {
      ...
      el.renderRoot.querySelector('sl-month-view:not([inert])')?.focus();
    ...
    it('should not change month when at the max boundary and using the keyboard', async () => {
      ...
      await el.updateComplete;
      const originalMonth = el.month;
      el.renderRoot.querySelector('sl-month-view:not([inert])')?.focus(new Date(2023, 2, 31));
  ...
  describe('navigation', () => {
    ...
    describe('back', () => {
      ...
      it('should scroll to the previous month when using the mouse', async () => {
        ...
        el.renderRoot.querySelector('sl-button.previous-month')?.click();
        await el.updateComplete;
    ...
    describe('forward', () => {
      ...
      it('should scroll to the next month when using the mouse', async () => {
        ...
        el.renderRoot.querySelector('sl-button.next-month')?.click();
        await scrollendPromise;
        expect(el.month).to.equalDate(new Date(2023, 3, 1));
      });
  ...
  describe('scrolling', () => {
    ...
    it('should update month views when month changes', async () => {
      ...
      await el.updateComplete;
      const monthViews = Array.from(el.renderRoot.querySelectorAll('sl-month-view'));
      expect(el.displayMonth).to.equalDate(new Date(2023, 5, 1));
      expect(el.previousMonth).to.equalDate(new Date(2023, 4, 1));
      expect(el.nextMonth).to.equalDate(new Date(2023, 6, 1));
...
//# sourceMappingURL=select-day.spec.js.map
