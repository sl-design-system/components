...
describe('sl-select-month', () => {
  ...
  describe('defaults', () => {
    ...
    it('should highlight the current month when show-current is set', async () => {
      ...
      const currentMonthButton = el.renderRoot.querySelector('button.current');
    ...
    it('should not have a selected month', () => {
      const selectedMonthButton = el.renderRoot.querySelector('button.selected');
    ...
    it('should show the selected month when set', async () => {
      el.selected = new Date(currentYear, 5, 1);
      await el.updateComplete;
      const selectedMonthButton = el.renderRoot.querySelector('button.selected');
  ...
  describe('navigation', () => {
    ...
    it('should increment year when next is clicked', async () => {
      el.renderRoot.querySelector('.arrows sl-button:last-of-type')?.click();
      await el.updateComplete;
      expect(el.month.getFullYear()).to.equal(currentYear + 1);
    });
    it('should decrement year when previous is clicked', async () => {
      el.renderRoot.querySelector('.arrows sl-button:first-of-type')?.click();
      await el.updateComplete;
      expect(el.month.getFullYear()).to.equal(currentYear - 1);
    });
  });
  ...
  describe('keyboard navigation', () => {
    ...
    describe('when min/max are set', () => {
      let buttons;
      beforeEach(async () => {
        el.min = new Date(month.getFullYear(), 3, 1);
        el.max = new Date(month.getFullYear(), 8, 1);
        await el.updateComplete;
  ...
  describe('dialog tab escape', () => {
    it('should not cycle back to the header when tabbing forward from the table', async () => {
      ...
      `);
      const monthPicker = dialog.querySelector('sl-select-month');
      await monthPicker.updateComplete;
      const rightArrow = monthPicker.renderRoot.querySelector('.arrows sl-button:last-of-type');
      rightArrow?.focus();
      await userEvent.keyboard('{Tab}');
      await monthPicker.updateComplete;
      await userEvent.keyboard('{Tab}');
      await monthPicker.updateComplete;
      const activeInsideShadow = monthPicker.shadowRoot?.activeElement;
      expect(activeInsideShadow).to.be.null;
    });
  });
});
//# sourceMappingURL=select-month.spec.js.map
