...
export default css`
  ...
  @layer grid-areas {
    :host {
      ...
      display: grid;
      grid-template: 'media' var(
          --_media-size
        ) 'header' max-content 'article' 1fr 'actions' max-content;
    }

    :host(:not(.sl-has-media)) {
      grid-template: 'header' max-content 'article' 1fr 'actions' max-content;
    }

    :host(:not(.sl-has-media)) figure {
      display: none;
      grid-area: unset;
    }
    ...
    :host(:where(.sl-horizontal.sl-has-media)) {
      ...
      grid-template-areas: 'media header' 'media article';
    }

    :host(:where(.sl-horizontal.sl-has-actions.sl-has-media)) {
      grid-template-areas: 'media header' 'media article' 'media actions';
    }
    ...
    :host(:where(.sl-horizontal.sl-has-media):not(.sl-has-actions, [subgrid])) {
      grid-template: 'media header' max-content 'media article' 1fr/var(--_media-size) 1fr;
    }
  ...
  @layer media {
    ...
    :host([fit-image][image-backdrop]) slot[name='media'] {
      background: transparent;
      z-index: 1;
    }

    :host([fit-image][image-backdrop]) figure .backdrop {
      ...
      opacity: var(--sl-opacity-new-700);
      position: absolute;
    }
  }
  ...
  @layer main-content {
    ...
    :host(.sl-has-actions) slot[name='actions'] {
      align-content: end;
    ...
    :host(.sl-has-menu-button) .header {
      padding-inline-end: calc(
        2 * var(--sl-size-100) + 1lh + (var(--sl-size-100) - var(--sl-size-borderWidth-action)) * 2
      );
      position: relative;
    }

    :host(.sl-has-menu-button) slot[name='menu-button'] {
      display: block;
