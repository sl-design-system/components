...
const card = (card2, contentId) => {
  return html`
    <sl-card
      orientation=${ifDefined(card2.orientation)}
      ?fit-image=${card2.fitImage}
      ?media-margin=${card2.mediaMargin}
      ?subgrid=${card2.subgrid}
      ?image-backdrop=${card2.imageBackdrop}>
      ${card2.media && card2.imageUrl ? html`<img slot="media" src=${images[contentId]} alt="Picture of ${titles[contentId]}" />` : nothing}
      ${card2.link ? html`<a href="javascript:console.log('link clicked');">${titles[contentId]}</a>` : html`<h2>${titles[contentId]}</h2>`}
      ${
        card2.subheaderContent
          ? html`
              <span slot="header">
                ${card2.subheaderBadge ? html`<sl-badge color="purple" size="lg">${card2.subheaderBadge}</sl-badge>` : nothing}
                ${card2.subheaderText}
              </span>
            `
          : nothing
      ...
      ${card2.bodyText ? html`<p slot="body">${bodyCopy[contentId]}</p>` : nothing}
      ${
        card2.actionButton
          ? html`
...
export const SubGridHorizontal = {
  args: {
    fitImage: true,
    subgrid: true
  },
  ...
  render: ({
    ...
    return html`
      ...
      <div class="grid${!actionButton ? ' no-buttons' : ''}">
        ...
        ${card({ ...settings, fitImage: true, bodyText: void 0 }, 3)}
...
export const LinkWithToggleButton = {
  render: () => {
    let linkClicks = 0,
      toggleClicks = 0;
    const updateCounters = root => {
    ...
    const onLinkClick = event => {
      ...
      linkClicks += 1;
      updateCounters(event.currentTarget.closest('.link-with-toggle'));
    };
...
export const All = {
  ...
  render: ({
    ...
    const vsettings = { ...settings, orientation: 'vertical' };
    return html`
      <style>
        .horizontal {
          display: grid;
