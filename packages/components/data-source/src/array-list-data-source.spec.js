...
import { people } from './list-data-source.spec.js';
describe('ArrayListDataSource', () => {
  ...
  describe('reordering', () => {
    it('should persist reordered items', () => {
      ds = new ArrayListDataSource(people);
      const ann = ds.items.find(item => isListDataSourceDataItem(item) && item.data.id === 1),
        bob = ds.items.find(item => isListDataSourceDataItem(item) && item.data.id === 32);
    ...
    it('should persist reordered items within a group', () => {
      ...
      const annJohnson = ds.items.find(
          item => isListDataSourceDataItem(item) && item.data.id === 3
        ),
        bob = ds.items.find(item => isListDataSourceDataItem(item) && item.data.id === 32);
      ds.reorder(bob, annJohnson, 'before');
      const gastroGroupMembers = ds.items
        .filter(
          item => isListDataSourceDataItem(item) && item.data.profession === 'Gastroenterologist'
        )
        .map(({ data }) => data.id);
    ...
    it('should keep items from filtered out groups when reordering visible groups', () => {
      ...
      ds.update();
      const visibleGroups = ds.items.filter(item => isListDataSourceGroupItem(item)),
...
//# sourceMappingURL=array-list-data-source.spec.js.map
