...
describe('sl-date-field', () => {
  ...
  describe('dialog', () => {
    ...
    it('should focus first spinbutton after calendar date selection', async () => {
      ...
      expect(el.renderRoot.activeElement).to.equal(firstSpan);
    });
  ...
  describe('keyboard entry', () => {
    ...
    beforeEach(async () => {
      el = await fixture(html`<sl-date-field aria-label="Date"></sl-date-field>`);
      spans = el.renderRoot.querySelectorAll('span[role="spinbutton"]');
    });
    ...
    it('should auto-advance to next input after max digits', async () => {
      ...
      await userEvent.keyboard('1');
      await userEvent.keyboard('2');
      expect(el.renderRoot.activeElement).to.equal(spans[1]);
    });
    ...
    it('should auto-advance from day to year', async () => {
      ...
      await userEvent.keyboard('1');
      await userEvent.keyboard('5');
      expect(el.renderRoot.activeElement).to.equal(spans[2]);
    });
    ...
    it('should move focus to previous input on ArrowLeft', async () => {
      ...
      expect(el.renderRoot.activeElement).to.equal(spans[0]);
    });
  ...
  describe('select all', () => {
    ...
    it('should focus the first spinbutton when exiting select-all via keydown', async () => {
      ...
      await userEvent.keyboard('{ArrowRight}');
      await el.updateComplete;
      const newSpans = el.renderRoot.querySelectorAll('span[role="spinbutton"]');
      expect(el.renderRoot.activeElement).to.equal(newSpans[0]);
    });
    ...
    it('should show placeholder text for empty parts in select-all mode', async () => {
      el = await fixture(html`<sl-date-field aria-label="Date"></sl-date-field>`);
      const emptyInputs = el.renderRoot.querySelectorAll('span[role="spinbutton"]');
      emptyInputs[0].focus();
      await userEvent.keyboard('3');
      await userEvent.keyboard('{Control>}a{/Control}');
      await el.updateComplete;
    ...
    it('should focus the field-button when pressing Tab in select-all mode', async () => {
      ...
      await userEvent.keyboard('{Tab}');
      await el.updateComplete;
      expect(el.renderRoot.activeElement).to.equal(el.renderRoot.querySelector('sl-field-button'));
    });
    ...
    describe('with requireConfirmation', () => {
      ...
      it('should not update value immediately when calendar date is selected', async () => {
        ...
        calendar.renderRoot
          ...
          ?.renderRoot.querySelector('button[part~="today"]')
          ?.click();
        expect(el.value).to.be.undefined;
        el.renderRoot.querySelector('sl-button')?.click();
        await el.updateComplete;
  ...
  describe('label association', () => {
    ...
    beforeEach(async () => {
      ...
      el = container.querySelector('sl-date-field');
    });
...
//# sourceMappingURL=date-field.spec.js.map
