...
describe('sl-dialog', () => {
  ...
  describe('defaults', () => {
    ...
    it('should have an accessible name for the close button', async () => {
      el.closeButton = true;
      await el.updateComplete;
      const closeButton = dialog.querySelector('sl-button.sl-close');
  ...
  describe('closing', () => {
    ...
    it('should close the dialog when the close button is clicked', async () => {
      const onClose = spy();
      el.addEventListener('sl-close', onClose);
      el.renderRoot.querySelector('sl-button.sl-close')?.click();
      await oneEvent(el, 'sl-close');
      expect(onClose).to.have.been.calledOnce;
    });
  ...
  describe('inheritance', () => {
    ...
    it('should render the given title passed to renderHeader()', async () => {
      ...
      const title = el2.renderRoot.querySelector('slot[name="title"]');
      expect(title).to.exist;
    ...
    it('should render the actions into the actions slot', async () => {
      ...
      const button = el2.renderRoot.querySelector('sl-button');
