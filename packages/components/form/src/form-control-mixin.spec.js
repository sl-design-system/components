...
function setShowValidity(el, value) {
  el.showValidity = value;
}
...
__decorateClass([property()], NativeTestElement.prototype, 'value', 2);
...
__decorateClass([property({ type: Boolean })], RequiredNativeTestElement.prototype, 'required', 2);
try {
...
describe('FormControlMixin', () => {
  ...
  describe('defaults', () => {
    ...
    it('should have a static extendsFormControlMixin property', () => {
      expect(NativeTestElement.extendsFormControlMixin).to.be.true;
    });
  ...
  describe('reportValidity', () => {
    ...
    it('should include validity info in sl-update-validity event', () => {
      let detail;
      requiredEl.addEventListener('sl-update-validity', event => {
        detail = event.detail;
      });
      requiredEl.reportValidity();
  ...
  describe('setCustomValidity', () => {
    ...
    describe('with a promise', () => {
      ...
      it('should not cause an infinite loop with async validation in sl-validate handler', async () => {
        vi.useFakeTimers();
        let validateCount = 0;
        el.addEventListener('sl-validate', event => {
          validateCount++;
          const target = event.target;
          const promise = new Promise(resolve => {
            setTimeout(() => resolve(target.value === 'SLDS' ? '' : 'Enter "SLDS"'), 2e3);
          });
...
//# sourceMappingURL=form-control-mixin.spec.js.map
