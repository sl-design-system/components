...
describe('sl-form', () => {
  ...
  describe('defaults', () => {
    ...
    it('should be valid if all controls are valid', async () => {
      ...
      el.querySelector('sl-text-field[name="bar"]')?.focus();
    ...
    it('should highlight required fields in the label', () => {
      const labels = Array.from(el.querySelectorAll('sl-label')).map(label =>
    ...
    it('should be dirty after modifying a field', async () => {
      el.querySelector('sl-text-field[name="foo"]')?.focus();
      await userEvent.keyboard('asdf');
    ...
    it('should be touched after focusing a field', () => {
      const input = el.querySelector('sl-text-field[name="bar"] input');
      input?.focus();
      input?.blur();
  ...
  describe('custom components', () => {
    ...
    it('should register the form controls INSIDE custom components', async () => {
      const customComponent2 = el.querySelector('custom-component');
      expect(customComponent2).to.exist;
      await el.updateComplete;
  ...
  describe('updateComplete', () => {
    ...
    it('should be invalid when the form is invalid', async () => {
      const textField = el.querySelector('sl-text-field[name="bar"]');
      if (textField) {
        textField.value = '';
      }
      await el.updateComplete;
    ...
    it('should handle multiple controls changing at once', async () => {
      const fooField = el.querySelector('sl-text-field[name="foo"]');
      const barField = el.querySelector('sl-text-field[name="bar"]');
      if (fooField) {
        fooField.value = '';
      }
      if (barField) {
