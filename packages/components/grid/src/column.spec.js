...
describe('sl-column', () => {
  ...
  describe('defaults', () => {
    ...
    it('should keep cell padding when ellipsized text adds a tooltip', async () => {
      ...
      el.requestUpdate();
      await el.updateComplete;
      const cell = el.renderRoot.querySelector('tbody tr:first-of-type td:first-of-type');
  ...
  describe('path', () => {
    ...
    it('should render "No path set" for the column that has no path', () => {
      const cells2 = Array.from(el.renderRoot.querySelectorAll('tbody td')).map(el2 =>
        el2.textContent?.trim()
      );
      expect(cells2).to.deep.equal(['Foo', '', 'No path set']);
    });
  });
  ...
  describe('empty string value', () => {
    ...
    it('should render nothing', () => {
      const data = Array.from(el.renderRoot.querySelectorAll('tbody td')).map(el2 =>
        el2.textContent?.trim()
      );
  ...
  describe('custom renderer', () => {
    beforeEach(async () => {
      const avatarRenderer = ({ firstName, lastName }) => {
        return html`
  ...
  describe('skeleton', () => {
    ...
    it('should render skeleton cells', () => {
      const skeletons = Array.from(el.renderRoot.querySelectorAll('td > *')).map(
        el2 => el2.tagName
      );
