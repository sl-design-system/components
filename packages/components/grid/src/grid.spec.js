...
describe('sl-grid', () => {
  ...
  const mountMultipleSelectGrid = async bulkActions => {
  ...
  describe('multiple select', () => {
    ...
    it('should toggle the "selected" part of the row when clicking in the selection column', async () => {
      el.renderRoot.querySelector('tbody tr:first-of-type td[part~="selection"]')?.click();
      await new Promise(resolve => setTimeout(resolve));
      const row = el.renderRoot.querySelector('tbody tr:first-of-type');
    ...
    it('should not toggle the "selected" part of the row when clicking anywhere in the row', async () => {
      el.renderRoot.querySelector('tbody tr:first-of-type td:last-of-type')?.click();
      await new Promise(resolve => setTimeout(resolve));
      const selectedRow = el.renderRoot.querySelector('tbody tr[part~="selected"]');
      expect(selectedRow).to.be.null;
    });
    it('should support multiple selection by clicking different rows', async () => {
      el.renderRoot.querySelector('tbody tr:first-of-type td[part~="selection"]')?.click();
      el.renderRoot.querySelector('tbody tr:last-of-type td[part~="selection"]')?.click();
      await new Promise(resolve => setTimeout(resolve));
      const rows = el.renderRoot.querySelectorAll('tbody tr[part~="selected"]');
  ...
  describe('grouping', () => {
    ...
    it('should allow dropping a row on a group header', async () => {
      ...
      const reorderSpy = spy(dataSource, 'reorder'),
        dragHandleCell = el.renderRoot.querySelector(
          "tbody tr:not([part~='group']) td[part*='drag-handle']"
        ),
        dataRow = dragHandleCell.closest('tr'),
        targetGroupRow = Array.from(el.renderRoot.querySelectorAll("tbody tr[part~='group']"))[1],
    ...
    it('should allow dragging and dropping a complete group', async () => {
      ...
      await waitForGridToRenderData(el);
      const reorderSpy = spy(dataSource, 'reorder'),
        groupRows = Array.from(el.renderRoot.querySelectorAll("tbody tr[part~='group']")),
    ...
    it('should update the row order immediately when reordering items within one group', async () => {
      ...
      const sourceRow = Array.from(
          el.renderRoot.querySelectorAll("tbody tr:not([part~='group'])")
        )[0],
        dragHandleCell = sourceRow.querySelector('td[part*="drag-handle"]'),
        targetRow = Array.from(el.renderRoot.querySelectorAll("tbody tr:not([part~='group'])"))[1],
        dragStartEvent = new DragEvent('dragstart', {
          bubbles: true,
          cancelable: true,
          dataTransfer: new DataTransfer()
  ...
  describe('single select', () => {
    ...
    it('should set aria-selected="true" on the selected row', async () => {
      ...
      await new Promise(resolve => setTimeout(resolve));
      const row = el.renderRoot.querySelector('tbody tr:first-of-type'),
        otherRow = el.renderRoot.querySelector('tbody tr:nth-of-type(2)');
    ...
    it('should allow only one row to be selected at a time', async () => {
      el.renderRoot.querySelector('tbody tr:first-of-type td:last-of-type')?.click();
      await new Promise(resolve => setTimeout(resolve));
      let selectedRows = el.renderRoot.querySelectorAll('tbody tr[part~="selected"]');
      expect(selectedRows).to.have.lengthOf(1);
      el.renderRoot.querySelector('tbody tr:nth-of-type(2) td:last-of-type')?.click();
      await new Promise(resolve => setTimeout(resolve));
      selectedRows = el.renderRoot.querySelectorAll('tbody tr[part~="selected"]');
      expect(selectedRows).to.have.lengthOf(1);
      const firstRow = el.renderRoot.querySelector('tbody tr:first-of-type');
      expect(firstRow?.part.contains('selected')).to.be.false;
      const secondRow = el.renderRoot.querySelector('tbody tr:nth-of-type(2)');
      expect(secondRow?.part.contains('selected')).to.be.true;
    });
    it('should deselect a row when clicking it again', async () => {
      el.renderRoot.querySelector('tbody tr:first-of-type td:last-of-type')?.click();
      await new Promise(resolve => setTimeout(resolve));
      let row = el.renderRoot.querySelector('tbody tr:first-of-type');
      expect(row?.part.contains('selected')).to.be.true;
      el.renderRoot.querySelector('tbody tr:first-of-type td:last-of-type')?.click();
      await new Promise(resolve => setTimeout(resolve));
      row = el.renderRoot.querySelector('tbody tr:first-of-type');
  ...
  describe('scrolling', () => {
    ...
    it('should sync the body when the header is scrolled', () => {
      const thead = el.renderRoot.querySelector('thead'),
        tbody = el.renderRoot.querySelector('tbody');
  ...
  describe('row action activate', () => {
    ...
    it('should add the "active" part to the active row', async () => {
      ...
      await el.updateComplete;
      const row = el.renderRoot.querySelector('tbody tr:last-of-type');
    ...
    it('should toggle the "active" part of the row when clicking in the row', async () => {
      ...
      tbody.querySelector('tr:last-of-type')?.click();
      await new Promise(resolve => setTimeout(resolve));
      expect(tbody.querySelector('tr:last-of-type')).to.match('[part~="active"]');
      expect(el.activeRow).to.deep.equal(el.items.at(1));
      tbody.querySelector('tr:last-of-type')?.click();
      await new Promise(resolve => setTimeout(resolve));
      expect(tbody.querySelector('tr:last-of-type')).to.not.match('[part~="active"]');
      expect(el.activeRow).to.be.undefined;
    });
    it('should emit an sl-grid-active-row-change event when clicking in the row', async () => {
      const onActiveRowChange = spy();
      el.addEventListener('sl-grid-active-row-change', onActiveRowChange);
      el.renderRoot.querySelector('tbody tr:last-of-type')?.click();
      await new Promise(resolve => setTimeout(resolve));
    ...
    it('should set aria-selected="true" on the active row', async () => {
      ...
      await new Promise(resolve => setTimeout(resolve));
      const row = el.renderRoot.querySelector('tbody tr:last-of-type'),
        otherRow = el.renderRoot.querySelector('tbody tr:first-of-type');
      expect(row).to.have.attribute('aria-selected', 'true');
      expect(otherRow).to.have.attribute('aria-selected', 'false');
    });
    it('should set aria-selected="false" when deactivating', async () => {
      el.renderRoot.querySelector('tbody tr:last-of-type')?.click();
      await new Promise(resolve => setTimeout(resolve));
      let row = el.renderRoot.querySelector('tbody tr:last-of-type');
      expect(row).to.have.attribute('aria-selected', 'true');
      row?.click();
      await new Promise(resolve => setTimeout(resolve));
      row = el.renderRoot.querySelector('tbody tr:last-of-type');
    ...
    it('should dispatch sl-announce event when activating a row', async () => {
      ...
      const event = announceSpy.firstCall.args[0];
    ...
    it('should dispatch sl-announce with force=true when focusing into an active row', async () => {
      el.renderRoot.querySelector('tbody tr:first-of-type')?.click();
      await new Promise(resolve => setTimeout(resolve));
      const tbody = el.renderRoot.querySelector('tbody');
      ...
      const announceSpy = spy();
      document.body.addEventListener('sl-announce', announceSpy);
      const td = el.renderRoot.querySelector('tbody tr:first-of-type td');
    ...
    it('should keep sticky active row cells opaque', async () => {
      ...
      await el.updateComplete;
      const cell = el.renderRoot.querySelector('tbody tr:first-of-type td.sticky-start-first');
  ...
  describe('row action select', () => {
    ...
    it('should support multiple selection by clicking different rows', async () => {
      el.renderRoot.querySelector('tbody tr:first-of-type td:last-of-type')?.click();
      el.renderRoot.querySelector('tbody tr:last-of-type td:last-of-type')?.click();
      await new Promise(resolve => setTimeout(resolve));
      const rows = el.renderRoot.querySelectorAll('tbody tr[part~="selected"]');
      expect(rows).to.have.lengthOf(2);
    });
  ...
  describe('bulk actions', () => {
    beforeEach(async () => {
      el = await fixture(html`
        <sl-grid
          ...
          .items=${[
            { firstName: 'Sophie', lastName: 'M\xFCller', email: 'sophie.muller@school1.edu' },
    ...
    it('should show the bulk-actions when rows are selected', async () => {
      ...
      const popover = el.renderRoot.querySelector('[part="bulk-actions"]');
    ...
    it('should have a tooltip on the cancel selection button', async () => {
      el.renderRoot.querySelector('tbody tr:first-of-type td[part~="selection"]')?.click();
      await new Promise(resolve => setTimeout(resolve, 100));
      const cancelButton = el.renderRoot.querySelector('[part="bulk-actions"] > sl-button');
