...
export const Activate = {
  render: (_, { loaded: { students } }) => {
    let activeStudent;
    const onActiveRowChange = ({ detail: student }) => {
...
export const MultipleInDialog = {
  tags: ['!dev'],
  // this is a scenario that is highly questionable, so we don't want to show it in the side bar, but keep it on hand if someone needs it
  args: {
    selectAll: false
  },
  render: ({ selectAll }, { loaded: { students } }) => {
  ...
  position-anchor: --grid-dialog;
      ...
      <sl-dialog id="invoker-dialog" close-button>
        ...
        <div style="flex:1; overflow: auto">
          <sl-grid .items=${students.slice(0, 30)}>
            <sl-grid-selection-column ?select-all=${selectAll}></sl-grid-selection-column>
            <sl-grid-column
              header="Student"
              path="fullName"
...
export const MultipleWithMenuButton = {
  args: {
    selectAll: false
  },
  render: ({ selectAll }, { loaded: { students } }) => {
...
export const WithLinks = {
  render: (_, { loaded: { students } }) => {
    ...
    const onSelectionChange = ({ detail: { grid } }) => {
      const selection = document.getElementById('selection'),
        selected = grid.dataSource?.selected ?? 0;
      if (selected > 0) {
        grid.activeRow = void 0;
        activeStudent = void 0;
