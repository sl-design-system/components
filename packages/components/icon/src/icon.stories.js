...
const sizes = ['2xs', 'xs', 'sm', 'md', 'lg', 'xl', '2xl', '3xl', '4xl'];
let storyId;
...
export const All = {
  render: () => {
    ...
    return html`
      ...
      <section class="copyable">
        ${icons.filter(i => window.SLDS.icons[i].type !== 'RegisteredIcon').map(i => html`<sl-icon .name=${i} size="2xl" .label=${i} title=${i}></sl-icon>`)}
      </section>
      <p>
...
export const RegisterAdditionalIcons = {
  render: () => {
    Icon.register(...Object.values(far));
    Icon.register(falPinata);
    Icon.register(falPinata);
