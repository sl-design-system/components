# @sl-design-system/link

## 0.0.3

### Patch Changes

- [#3694](https://github.com/sl-design-system/components/pull/3694) [`9417d4a`](https://github.com/sl-design-system/components/commit/9417d4ab0e2f0a2df800db4be713bc4cb7c299c3) - Fix published imports to resolve built JavaScript and type declarations by default, while preserving local source imports through a custom export condition.

- Updated dependencies [[`9417d4a`](https://github.com/sl-design-system/components/commit/9417d4ab0e2f0a2df800db4be713bc4cb7c299c3)]:
  - @sl-design-system/icon@1.4.5

## 0.0.2

### Patch Changes

- [#3664](https://github.com/sl-design-system/components/pull/3664) [`0dfa9a8`](https://github.com/sl-design-system/components/commit/0dfa9a84b1fa83d9b38d0b0145fa0e4f7731101d) - Fixed an issue where no-icon also removed padding for types other than internal but still showed the icon, causing it to overlap the text.

- Updated dependencies [[`07bc4e5`](https://github.com/sl-design-system/components/commit/07bc4e59839582242bda1dddbea1dda5cd404652), [`07bc4e5`](https://github.com/sl-design-system/components/commit/07bc4e59839582242bda1dddbea1dda5cd404652)]:
  - @sl-design-system/icon@1.4.4

## 0.0.1

### Patch Changes

- [#3590](https://github.com/sl-design-system/components/pull/3590) [`df49009`](https://github.com/sl-design-system/components/commit/df490090ace75a4463a3f9f3e877b4b8d7e8957f) - First version of the standalone link component
