...
describe('sl-menu-item-group', () => {
  ...
  it('should toggle the selected menu item when single select is set', async () => {
    el.selects = 'single';
    const item1 = el.querySelector('sl-menu-item'),
      item2 = el.querySelector('sl-menu-item:last-child');
    item1?.click();
    await el.updateComplete;
  ...
  it('should update aria-label when slotted header element is replaced', async () => {
    ...
    const headerSlot = el.renderRoot.querySelector('slot[name="header"]');
    if (!headerSlot) {
...
//# sourceMappingURL=menu-item-group.spec.js.map
