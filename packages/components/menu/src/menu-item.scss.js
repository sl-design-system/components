...
export default css`
  ...
  :host([variant='danger'])::before {
    background: var(--sl-color-background-negative-bold);
  }

  :host([variant='danger'][selected]) {
    --sl-menu-item-indent: 0;
  ...
  :host(:is([disabled], [aria-disabled='true'])) .wrapper {
    cursor: default;
  }
  ...
  .safe-triangle:hover + [part='wrapper'] {
  ...
  [part='wrapper'] {
    ...
    padding-inline-start: calc(
      var(--sl-menu-item-indent, 0) * (var(--sl-size-new-icon-sm) + var(--sl-size-100)) +
        var(--sl-size-100)
    );
    position: relative;
  }
  ...
  :host(:not([selected], [variant='danger'])) [part='wrapper']:hover,
  :host(:not([selected], [variant='danger'])) .safe-triangle:hover + [part='wrapper'] {
