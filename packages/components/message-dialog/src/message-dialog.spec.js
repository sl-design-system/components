...
describe('sl-message-dialog', () => {
  ...
  describe('alert', () => {
    ...
    it('should resolve the promise with undefined when the dialog is cancelled', async () => {
      ...
      expect(callback).to.have.been.calledWith(void 0);
    });
    it('should resolve the promise when the OK button is clicked', async () => {
      const callback = spy();
      promise.then(callback);
      dialog.querySelector('sl-button')?.click();
  ...
  describe('confirm', () => {
    ...
    it('should resolve the promise with true when the OK button is clicked', async () => {
      const callback = spy();
      promise.then(callback);
      dialog.querySelector('sl-button:last-of-type')?.click();
      await new Promise(resolve => setTimeout(resolve));
  ...
  describe('show', () => {
    ...
    it('should have a custom message', () => {
      const html2 = dialog.querySelector('p')?.innerHTML;
      expect(html2).to.match(/This is a message with <strong>HTML<\/strong>!/);
    });
    ...
    it('should resolve the promise with "YES" when the danger button is clicked', async () => {
      const callback = spy();
      promise.then(callback);
      dialog.querySelector('sl-button[variant="danger"]')?.click();
      await new Promise(resolve => setTimeout(resolve, 50));
      expect(callback).to.have.been.calledWith('YES');
    });
  });
});
//# sourceMappingURL=message-dialog.spec.js.map
