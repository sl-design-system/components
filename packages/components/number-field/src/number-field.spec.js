...
describe('sl-number-field', () => {
  ...
  describe('step buttons', () => {
    ...
    it('should increase the value when step up button is clicked', async () => {
      const button = el.renderRoot.querySelector('sl-field-button[aria-label="Step up"]');
    ...
    it('should decrease the value when step down button is clicked', async () => {
      const button = el.renderRoot.querySelector('sl-field-button[aria-label="Step down"]');
      expect(button).to.exist;
      button.click();
      await el.updateComplete;
...
//# sourceMappingURL=number-field.spec.js.map
