...
describe('sl-paginator', () => {
  ...
  describe('defaults', () => {
    ...
    it('should have the current page set to the first page', () => {
      const button = el.renderRoot.querySelector('sl-button.current');
      expect(button).to.exist;
    ...
    it('should render the current page as a primary outline button by default', () => {
      const currentPage = el.renderRoot.querySelector('sl-button.current'),
        otherPage = el.renderRoot.querySelector(':nth-child(2 of sl-button.page)');
    ...
    it('should only show 10 pages', () => {
      const buttons = Array.from(el.renderRoot.querySelectorAll('sl-button.page')).filter(
    ...
    it('should update the current page when a page button is clicked', async () => {
      el.renderRoot.querySelector(':nth-child(3 of sl-button.page)')?.click();
      await el.updateComplete;
    ...
    it('should move focus to the selected page when a menu item is clicked', async () => {
      ...
      const opened = new Promise(resolve =>
        menuButton.addEventListener('sl-toggle', event => resolve(event), {
          once: true
        })
      );
      trigger.click();
    ...
    it('should decrement the current page when the previous button is clicked', async () => {
      el.page = 10;
      await el.updateComplete;
      el.renderRoot.querySelector(':nth-child(1 of sl-button.nav')?.click();
      await el.updateComplete;
      expect(el.page).to.equal(9);
      const currentPage = el.renderRoot.querySelector('sl-button.current');
      expect(getForwardedAccessibleName(currentPage)).to.equal('10');
    ...
    it('should emit an sl-page-change event when the page has changed', async () => {
      ...
      el.renderRoot.querySelector(':nth-child(5 of sl-button.page)')?.click();
      await el.updateComplete;
  ...
  describe('announcement', () => {
    ...
    it('should announce the current page after clicking the next button', async () => {
      const announce = spy();
      document.body.addEventListener('sl-announce', announce);
      el.renderRoot.querySelector(':nth-child(2 of sl-button.nav)')?.click();
      await el.updateComplete;
      expect(announce).to.have.been.calledOnce;
  ...
  describe('width', () => {
    ...
    describe('visible pages', () => {
      ...
      it('should not show any buttons when the width is xs', async () => {
        el.width = 'xs';
        await el.updateComplete;
        const buttons = Array.from(el.renderRoot.querySelectorAll('sl-button')).filter(
      ...
      it('should not wrap page numbers in select options when the width is xs', async () => {
        el.width = 'xs';
        await el.updateComplete;
        const option = el.renderRoot.querySelector('sl-option:nth-of-type(10)'),
