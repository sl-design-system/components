...
describe('sl-panel', () => {
  ...
  describe('defaults', () => {
    ...
    it('should render the heading into the heading slot', () => {
      const heading = el.renderRoot.querySelector('slot[name="heading"]');
  ...
  describe('collapsible', () => {
    ...
    it('should hide initially collapsed body from assistive technology', async () => {
      const collapsedEl = await fixture(
  ...
  describe('slotted elements', () => {
    ...
    it('should slot the heading into the heading slot', () => {
      const elements = el.renderRoot
        .querySelector('slot[name="heading"]')
  ...
  describe('animations', () => {
    it('should manage no-transition attribute during lifecycle', async () => {
      ...
      await new Promise(resolve => requestAnimationFrame(resolve));
      expect(
        el2.hasAttribute('no-transition'),
    ...
    it('should disable transitions when no-transition attribute is present', async () => {
      ...
      expect(el2.hasAttribute('no-transition'), 'no-transition should be removed initially').to.be
        .false;
      const body = el2.renderRoot.querySelector('[part="body"]'),
        initialProperty = getComputedStyle(body).transitionProperty,
        initialDuration = getComputedStyle(body).transitionDuration;
      el2.setAttribute('no-transition', '');
      await el2.updateComplete;
      ...
      el2.removeAttribute('no-transition');
      await el2.updateComplete;
    ...
    it('should animate normally when toggling after initialization', async () => {
      ...
      `;
      el2.shadowRoot.appendChild(fallbackStyle);
      for (let i = 0; i < 3; i++) {
        await new Promise(resolve => requestAnimationFrame(resolve));
      }
      const body = el2.renderRoot.querySelector('[part="body"]');
