...
export default css`
  ...
  :host([disabled]) [part='placeholder'] {
    color: var(--sl-color-foreground-disabled);
  }
  ...
  :host([aria-expanded='true']) .status sl-icon {
    rotate: 180deg;
  }
  ...
  :host(:state(clearable):state(has-selection)) .wrapper {
    padding-inline-end: calc(1lh + (var(--sl-size-100) - var(--sl-size-borderWidth-default)) * 2);
  }

  :host(:state(clearable):state(has-selection)) .status {
    border-inline-start: var(--sl-size-borderWidth-default) solid
      var(--sl-color-border-neutral-subtle);
  ...
  :host(:state(clearable):state(has-selection)[size='lg']) .wrapper {
    padding-inline-end: calc(1lh + (var(--sl-size-175) - var(--sl-size-borderWidth-default)) * 2);
  }

  ::slotted(span[slot='selected-content']) {
    display: contents;
  }
...
`;
//# sourceMappingURL=select-button.scss.js.map
