...
describe('sl-select', () => {
  ...
  describe('defaults', () => {
    ...
    it('should have a value after selection', async () => {
      const button2 = el.querySelector('sl-select-button');
      button2?.click();
      await el.updateComplete;
    ...
    it('should be dirty after clicking an option', async () => {
      el.querySelector('sl-select-button')?.click();
      await el.updateComplete;
      el.querySelector('sl-option')?.click();
      await el.updateComplete;
  ...
  describe('groups', () => {
    ...
    it('should be able to navigate options that were added later', async () => {
      ...
      option.value = '5';
      el.querySelector('sl-option-group:last-of-type').appendChild(option);
      await new Promise(resolve => setTimeout(resolve, 50));
  ...
  describe('form integration', () => {
    ...
    it('should focus the button when the label is clicked', async () => {
      const button2 = el2.renderRoot.querySelector('sl-select-button'),
        label = el2.renderRoot.querySelector('label');
      label?.click();
      await el2.updateComplete;
      expect(el2.shadowRoot.activeElement).to.equal(button2);
    });
    it('should set ariaLabelledByElements on the select button from associated labels', async () => {
      const select = el2.renderRoot.querySelector('sl-select'),
        button2 = select.querySelector('sl-select-button'),
        labels = Array.from(select.internals.labels);
      await new Promise(resolve => requestAnimationFrame(() => resolve(void 0)));
      expect(labels.length).to.equal(1);
      expect(button2.ariaLabelledByElements).to.deep.equal(labels);
    });
  ...
  describe('keyboard interactions', () => {
    ...
    it('should stop Escape key propagation when the listbox is open', async () => {
      ...
      await userEvent.keyboard('{Escape}');
      await el.updateComplete;
      const escapeEvents = onKeydown.getCalls().filter(call => call.args[0].key === 'Escape');
  ...
  describe('selected content rendering', () => {
    ...
    it('should batch largest option width recalculation while a frame is pending', async () => {
      ...
      const getOptionSize = optionSizeDescriptor.get.bind(button);
      const setOptionSize = optionSizeDescriptor.set.bind(button);
      try {
    ...
    it('should sync value and form value when selected option implicit value changes', async () => {
      ...
      `);
      el = form.querySelector('sl-select');
      button = el.querySelector('sl-select-button');
      el.value = 'Apple';
      await el.updateComplete;
      const onChange = spy();
    ...
    it('should upgrade any cloned custom elements of the selected option', async () => {
      ...
      const wrapper = await fixture(html`<scoped-select-wrapper></scoped-select-wrapper>`);
  ...
  describe('explicit aria-label/aria-labelledby handling', () => {
    it('should not override explicit aria-label on the button with associated labels', async () => {
      ...
      `);
      const select = wrapper.querySelector('sl-select'),
        button2 = select.querySelector('sl-select-button');
      await new Promise(resolve => requestAnimationFrame(() => resolve(void 0)));
      expect(button2).to.have.attribute('aria-label', 'Explicit label');
      expect(button2.ariaLabelledByElements ?? []).to.have.length(0);
    ...
    it('should propagate explicit aria-labelledby to the listbox via ariaLabelledByElements', async () => {
      ...
      `);
      const select = wrapper.querySelector('sl-select'),
        explicitLabel = wrapper.querySelector('#explicit-label');
      await new Promise(resolve => requestAnimationFrame(() => resolve(void 0)));
    ...
    it('should keep listbox aria-labelledby in sync after later aria-labelledby changes', async () => {
      ...
      `);
      const select = wrapper.querySelector('sl-select'),
        label1 = wrapper.querySelector('#label-1'),
        label2 = wrapper.querySelector('#label-2');
      await new Promise(resolve => requestAnimationFrame(() => resolve(void 0)));
    ...
    it('should handle multiple aria-labelledby IDs separated by spaces', async () => {
      ...
      `);
      const select = wrapper.querySelector('sl-select'),
        labelA = wrapper.querySelector('#label-a'),
        labelB = wrapper.querySelector('#label-b');
      await new Promise(resolve => requestAnimationFrame(() => resolve(void 0)));
    ...
    it('should filter out non existing aria-labelledby IDs', async () => {
      ...
      `);
      const select = wrapper.querySelector('sl-select'),
        labelExists = wrapper.querySelector('#label-exists');
      await new Promise(resolve => requestAnimationFrame(() => resolve(void 0)));
