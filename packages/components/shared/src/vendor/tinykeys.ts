...
/** Keybinding sequences should timeout if individual key presses are more than 1s apart by default. */
...
/**
 * Meaning of `AltGraph`, from MDN: - Windows: Both Alt and Ctrl keys are pressed, or AltGr key is
 * pressed - Mac: ⌥ Option key pressed - Linux: Level 3 Shift key (or Level 5 Shift key) pressed -
...
/**
 * There's a bug in Chrome that causes event.getModifierState not to exist on KeyboardEvent's for
 * F1/F2/etc keys.
 */
...
/** This tells us if a series of events matches a key binding sequence either partially or exactly. */
