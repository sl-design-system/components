...
describe('sl-switch', () => {
  ...
  describe('defaults', () => {
    ...
    it('should label the input when the switch re-projects content via a forwarded named slot', async () => {
      // Create a wrapper component that forwards its named slot to the switch
      class WrapperElement extends LitElement {
        override render(): TemplateResult {
          return html`<sl-switch><slot name="description"></slot></sl-switch>`;
        }
      }
      customElements.define('test-switch-wrapper-named-slot', WrapperElement);

      const wrapper = await fixture<WrapperElement>(
        html`<test-switch-wrapper-named-slot
          ><span slot="description">Forwarded description</span></test-switch-wrapper-named-slot
        >`
      );
      await wrapper.updateComplete;
      ...
      // The input should be labeled by the switch's label element
      expect(switchInput).not.to.have.attribute('aria-labelledby', 'label');
    });
    ...
    it('should label the input when the switch re-projects content via a forwarded named slot', async () => {
      ...
      class WrapperElement extends LitElement {
        override render(): TemplateResult {
          return html`<sl-switch><slot name="description"></slot><slot></slot></sl-switch>`;
        }
      }
      customElements.define('test-switch-wrapper-two-slots', WrapperElement);

      const wrapper = await fixture<WrapperElement>(
        html`<test-switch-wrapper-two-slots
          ><span slot="description">Forwarded description</span>Forwarded
          label</test-switch-wrapper-two-slots
        >`
      );
      await wrapper.updateComplete;
