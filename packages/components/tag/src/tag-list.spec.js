...
describe('sl-tag-list', () => {
  let el;
  const triggerVisibilityUpdate = async () => {
  ...
  describe('removable', () => {
    ...
    it('should restore removable tag disabled states when the list is enabled again', async () => {
      ...
      el.disabled = false;
      await el.updateComplete;
      expect(tags.map(tag => tag.disabled)).to.deep.equal([void 0, true, void 0]);
    });
    ...
    it('should use a single tab stop for removable tag buttons', async () => {
      ...
      `).then(wrapper2 => wrapper2.querySelector('sl-tag-list'));
      await new Promise(resolve => requestAnimationFrame(() => resolve()));
    ...
    it('should allow tabbing through removable tag buttons when keyboard navigation is disabled', async () => {
      ...
      expect(tags.map(tag => tag.navigationDescription)).to.deep.equal([void 0, void 0, void 0]);
      before.focus();
      await userEvent.tab();
      expect(document.activeElement).to.equal(tags[0]);
      expect(tags[0].shadowRoot?.activeElement).to.equal(buttons[0]);
  ...
  describe('stacked', () => {
    ...
    it('should have a tooltip for the stack', () => {
      const tag = el.renderRoot.querySelector('sl-tag.stack'),
    ...
    it('should have hidden tags with tabindex -1', async () => {
      ...
      const tabindexes = [
          tag.tabIndex,
          ...Array.from(el.querySelectorAll('sl-tag')).map(tag2 => tag2.tabIndex)
        ],
        visibility = [
          getComputedStyle(tag).display,
          ...Array.from(el.querySelectorAll('sl-tag')).map(tag2 => getComputedStyle(tag2).display)
        ];
    ...
    it('should update the stack size when a tag is removed', async () => {
      const tag = el.renderRoot.querySelector('sl-tag');
      expect(tag).to.have.trimmed.text('+7');
      el.querySelector('sl-tag:last-child')?.focus();
      await userEvent.keyboard('{Backspace}');
  ...
  describe('visibility edge cases', () => {
    ...
    it('should restore a constrained tag max-inline-size before measuring again', async () => {
      ...
      await triggerVisibilityUpdate();
      const longTag = el.querySelectorAll('sl-tag')[1];
...
//# sourceMappingURL=tag-list.spec.js.map
