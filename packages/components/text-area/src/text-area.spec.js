...
describe('sl-text-area', () => {
  ...
  describe('form integration', () => {
    ...
    it('should focus the textarea when the label is clicked', async () => {
      const textarea = el2.renderRoot.querySelector('textarea'),
        label = el2.renderRoot.querySelector('label');
      label?.click();
      await el2.updateComplete;
      expect(el2.shadowRoot.activeElement).to.equal(textarea);
    });
  });
  describe('show-count', () => {
    beforeEach(async () => {
    ...
    it('should connect count span to textarea with ariaDescribedByElements when count is visible', async () => {
      ...
      expect(linkedElements.some(el2 => el2.textContent?.trim() === '5 characters remaining')).to.be
        .true;
    });
    it('should include hint element in ariaDescribedByElements when both hint and count are present', async () => {
      ...
      expect(list.some(el2 => el2.id === 'test-hint-123')).to.be.true;
      expect(list.some(el2 => el2.textContent?.trim() === '5 characters remaining')).to.be.true;
      hintEl.remove();
    });
    it('should preserve existing aria-describedby ids when count is visible', async () => {
      ...
      expect(refs.some(el2 => el2.id === 'sl-form-field-hint-1')).to.be.true;
      expect(refs.some(el2 => el2.id.endsWith('-description'))).to.be.true;
      hintEl.remove();
    });
    it('should refresh external aria-describedby refs when ids change while count is visible', async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 50));
      const refs = Array.from(textarea.ariaDescribedByElements ?? []);
      expect(refs.some(el2 => el2.id === hintOne.id)).to.be.false;
      expect(refs.some(el2 => el2.id === hintTwo.id)).to.be.true;
      expect(refs.some(el2 => el2.id.endsWith('-description'))).to.be.true;
      hintOne.remove();
      hintTwo.remove();
    });
    it('should keep count description element in accessibility tree', async () => {
      ...
      const description = refs.find(
        el2 => el2.id.includes('sl-text-area-count-') && el2.id.endsWith('-description')
      );
    ...
    it('should expose count in ariaDescribedByElements when used inside form-field', async () => {
      ...
      `);
      const area = wrapped.querySelector('sl-text-area'),
