...
describe('FlatTreeDataSource', () => {
  ...
  describe('lazy loading children', () => {
    ...
    it('should handle lazy loading errors gracefully', async () => {
      ...
      const errorDs = new FlatTreeDataSource(
        [{ id: 1, name: 'Parent 1', level: 0, expandable: true }],
        {
          getId: ({ id }) => id,
          getLabel: ({ name }) => name,
  ...
  describe('selectable', () => {
    ...
    it('should mark all nodes as selectable by default', () => {
      ds = new FlatTreeDataSource(items, {
        getId: ({ id }) => id,
        getLabel: ({ name }) => name,
        getLevel: ({ level }) => level,
        isExpandable: ({ expandable }) => expandable,
...
//# sourceMappingURL=flat-tree-data-source.spec.js.map
