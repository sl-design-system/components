...
describe('NestedTreeDataSource', () => {
  ...
  describe('lazy loading children', () => {
    ...
    beforeEach(() => {
      ...
      ds = new NestedTreeDataSource(
        ...
        ],
        {
          ...
          getLabel: ({ name }) => name,
          getChildren: ({ children }) => children,
          isExpandable: item => item.id === 1 || item.id === 2,
          // Only items 1 and 2 are expandable
          isExpanded: ({ expanded }) => expanded ?? false,
    ...
    it('should handle lazy loading errors gracefully', async () => {
      ...
      const errorDs = new NestedTreeDataSource([{ id: 1, name: 'Parent 1' }], {
        getId: ({ id }) => id,
        getLabel: ({ name }) => name,
        getChildren: ({ children }) => children,
        isExpandable: () => true,
      ...
      try {
        ...
        expect(error.message).to.equal('Failed to load children');
      }
      expect(parentNode.children).to.be.undefined;
      expect(parentNode.childrenLoading).to.be.instanceof(Promise);
    });
    it('should select all lazy loaded children when parent is already selected', async () => {
      ...
      expect(children[0].selected).to.be.true;
      expect(children[1].selected).to.be.true;
  ...
  describe('selectable', () => {
    ...
    const createDataSource = (overrides = {}) =>
      new NestedTreeDataSource(items, {
        getId: ({ id }) => id,
        getLabel: ({ name }) => name,
        getChildren: ({ children }) => children,
        isExpandable: ({ children }) => !!children?.length,
    ...
    it('should mark all nodes as selectable by default', () => {
      ds = new NestedTreeDataSource(items, {
        getId: ({ id }) => id,
        getLabel: ({ name }) => name,
        getChildren: ({ children }) => children,
        isExpandable: ({ children }) => !!children?.length,
    ...
    it('should only select the selectable children when selecting a parent', () => {
      ...
      const subA = ds.items.find(n => n.id === 11),
        ...
        leaves = ds.items.filter(n => [111, 112, 121].includes(n.id));
      expect(parent.selected).to.be.true;
    ...
    it('should only select selectable nodes when calling selectAll', () => {
      ...
      ds.update();
      ds.selectAll();
      const leaves = ds.items.filter(n => [111, 112, 121].includes(n.id));
      expect(leaves.every(n => !n.selected)).to.be.true;
      expect(ds.selection.size).to.equal(3);
    });
  });
});
//# sourceMappingURL=nested-tree-data-source.spec.js.map
