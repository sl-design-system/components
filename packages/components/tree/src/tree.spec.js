...
describe('sl-tree', () => {
  ...
  describe('accessibility', () => {
    ...
    beforeEach(async () => {
      ds = new NestedTreeDataSource(nestedData, {
        getAriaDescription: ({ description }) => description ?? void 0,
  ...
  describe('keyboard navigation', () => {
    ...
    it('should have a tabindex of -1 for all other nodes', () => {
      const tabIndices = Array.from(
        el.renderRoot.querySelectorAll('sl-tree-node:not(:first-of-type)')
    ...
    it('should focus the parent node when pressing the left arrow on a leaf node', async () => {
      el.renderRoot.querySelector('sl-tree-node:nth-of-type(5)')?.focus();
    ...
    it('should do nothing when pressing the right arrow on a leaf node', async () => {
      el.renderRoot.querySelector('sl-tree-node:nth-of-type(4)')?.focus();
    ...
    it('should expand all siblings that are at the same level when pressing *', async () => {
      el.renderRoot.querySelector('sl-tree-node:nth-of-type(2)')?.focus();
  ...
  describe('using flat data', () => {
    ...
    it('should render the tree nodes with the correct icons', () => {
      const icons = Array.from(el.renderRoot.querySelectorAll('sl-tree-node sl-icon')).map(
        icon => icon.name
      );
  ...
  describe('keyboard navigation with custom content', () => {
    ...
    it('should focus the next tree node when pressing ArrowDown while a button in the node has focus', async () => {
      const node1 = el.renderRoot.querySelector('sl-tree-node[data-index="1"]'),
        btn1 = node1.querySelector('#btn-1');
      btn1.focus();
      await userEvent.keyboard('{ArrowDown}');
