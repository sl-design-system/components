...
export default {
  ...
  render: ({ estimateSize, gap, itemCount, items, overflow, overscan, renderItem, behavior }) => {
    items ??= Array.from({ length: itemCount ?? 1e4 }, (_, i) => ({
      id: i,
      name: `Item ${i}`,
    ...
    renderItem ??= item => html`
      <div
        part="item-content"
        style=${item.id === scrollToPosition ? '--_outline-color: var(--sl-color-border-focused)' : ''}>
...
//# sourceMappingURL=virtual-list.stories.js.map
