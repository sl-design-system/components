export const icons = {
  'angle-down': {
    value: 'angle-down',
    type: 'Icons',
    description: 'icon.core.angle-down',
    svg: '<svg viewBox="0 0 448 512" xmlns="http://www.w3.org/2000/svg"><path d="M241 369c-9.4 9.4-24.6 9.4-33.9 0L47 209c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0l143 143L367 175c9.4-9.4 24.6-9.4 33.9 0s9.4 24.6 0 33.9L241 369z" fill="var(--sl-icon-fill-default)"></path></svg>'
  },
  'breadcrumb-separator': {
    value: 'slash-forward',
    type: 'Icons',
    description: 'icon.core.breadcrumb-separator',
    svg: '<svg viewBox="0 0 320 512" xmlns="http://www.w3.org/2000/svg"><path d="M308.1 3.3c11.4 6.7 15.3 21.4 8.6 32.8l-272 464c-6.7 11.4-21.4 15.3-32.8 8.6S-3.4 487.3 3.3 475.9l272-464C282 .4 296.7-3.4 308.1 3.3z" fill="var(--sl-icon-fill-default)"></path></svg>'
  },
  check: {
    value: 'check',
    type: 'Icons',
    description: 'icon.core.check',
    svg: '<svg viewBox="0 0 448 512" xmlns="http://www.w3.org/2000/svg"><path d="M441 103c9.4 9.4 9.4 24.6 0 33.9L177 401c-9.4 9.4-24.6 9.4-33.9 0L7 265c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0l119 119L407 103c9.4-9.4 24.6-9.4 33.9 0z" fill="var(--sl-icon-fill-default)"></path></svg>'
  },
  'check-solid': {
    value: 'check',
    type: 'Icons',
    description: 'icon.core.check-solid',
    svg: '<svg viewBox="0 0 448 512" xmlns="http://www.w3.org/2000/svg"><path d="M438.6 105.4c12.5 12.5 12.5 32.8 0 45.3l-256 256c-12.5 12.5-32.8 12.5-45.3 0l-128-128c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0L160 338.7 393.4 105.4c12.5-12.5 32.8-12.5 45.3 0z" fill="var(--sl-icon-fill-default)"></path></svg>'
  },
  'chevron-down': {
    value: 'chevron-down',
    type: 'Icons',
    description: 'icon.core.chevron-down',
    svg: '<svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="M239 401c9.4 9.4 24.6 9.4 33.9 0L465 209c9.4-9.4 9.4-24.6 0-33.9s-24.6-9.4-33.9 0l-175 175L81 175c-9.4-9.4-24.6-9.4-33.9 0s-9.4 24.6 0 33.9L239 401z" fill="var(--sl-icon-fill-default)"></path></svg>'
  },
  'chevron-right': {
    value: 'chevron-right',
    type: 'Icons',
    description: 'icon.core.chevron-right',
    svg: '<svg viewBox="0 0 320 512" xmlns="http://www.w3.org/2000/svg"><path d="M305 239c9.4 9.4 9.4 24.6 0 33.9L113 465c-9.4 9.4-24.6 9.4-33.9 0s-9.4-24.6 0-33.9l175-175L79 81c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0L305 239z" fill="var(--sl-icon-fill-default)"></path></svg>'
  },
  circle: {
    svg: '<svg fill="var(--sl-icon-fill-default)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><circle cx="8" cy="8" r="8"/></svg>'
  },
  'circle-check-solid': {
    value: 'circle-check',
    type: 'Icons',
    description: 'icon.core.circle-check-solid',
    svg: '<svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM369 209L241 337c-9.4 9.4-24.6 9.4-33.9 0l-64-64c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0l47 47L335 175c9.4-9.4 24.6-9.4 33.9 0s9.4 24.6 0 33.9z" fill="var(--sl-icon-fill-default)"></path></svg>'
  },
  'circle-exclamation-solid': {
    value: 'circle-exclamation',
    type: 'Icons',
    description: 'icon.core.circle-exclamation-solid',
    svg: '<svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zm0-384c13.3 0 24 10.7 24 24l0 112c0 13.3-10.7 24-24 24s-24-10.7-24-24l0-112c0-13.3 10.7-24 24-24zM224 352a32 32 0 1 1 64 0 32 32 0 1 1 -64 0z" fill="var(--sl-icon-fill-default)"></path></svg>'
  },
  ellipsis: {
    value: 'ellipsis',
    type: 'Icons',
    description: 'icon.core.ellipsis',
    svg: '<svg viewBox="0 0 448 512" xmlns="http://www.w3.org/2000/svg"><path d="M432 256a48 48 0 1 1 -96 0 48 48 0 1 1 96 0zm-160 0a48 48 0 1 1 -96 0 48 48 0 1 1 96 0zM64 304a48 48 0 1 1 0-96 48 48 0 1 1 0 96z" fill="var(--sl-icon-fill-default)"></path></svg>'
  },
  'ellipsis-down': {
    svg: '<svg fill="var(--sl-icon-fill-default)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path d="M14.512 12.5c0 .563-.313 1.031-.75 1.313-.469.28-1.063.28-1.5 0a1.5 1.5 0 0 1-.75-1.313c0-.531.281-1 .75-1.281.437-.281 1.031-.281 1.5 0 .437.281.75.75.75 1.281m-5 0c0 .563-.313 1.031-.75 1.313-.469.28-1.063.28-1.5 0a1.5 1.5 0 0 1-.75-1.313c0-.531.281-1 .75-1.281.437-.281 1.031-.281 1.5 0 .437.281.75.75.75 1.281m-6.5 1.5a1.5 1.5 0 0 1-1.313-.75c-.281-.437-.281-1.031 0-1.5.281-.437.75-.75 1.313-.75.531 0 1 .313 1.281.75.281.469.281 1.063 0 1.5-.281.469-.75.75-1.281.75"/></svg>'
  },
  eye: {
    svg: '<svg fill="var(--sl-icon-fill-default)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path d="M4.455 8a3.565 3.565 0 0 1 3.559-3.559c1.946 0 3.559 1.613 3.559 3.559a3.565 3.565 0 0 1-3.56 3.559A3.55 3.55 0 0 1 4.456 8m3.559 2.224A2.213 2.213 0 0 0 10.238 8c0-1.223-1-2.224-2.224-2.224H7.93c.055.167.083.306.083.445 0 1-.806 1.779-1.78 1.779-.166 0-.305 0-.444-.056V8c0 1.251.973 2.224 2.224 2.224m-5.366-6.2c1.307-1.223 3.114-2.252 5.366-2.252 2.224 0 4.031 1.029 5.338 2.252 1.307 1.196 2.168 2.641 2.585 3.642a1 1 0 0 1 0 .695c-.417.973-1.278 2.42-2.585 3.642-1.307 1.224-3.114 2.225-5.338 2.225-2.252 0-4.06-1.001-5.366-2.225C1.341 10.78.48 9.334.063 8.361a1 1 0 0 1 0-.695c.417-1 1.278-2.446 2.585-3.642m5.366-.917c-1.835 0-3.309.834-4.448 1.89A9.9 9.9 0 0 0 1.369 8c.39.834 1.112 2.03 2.197 3.03 1.14 1.057 2.613 1.863 4.448 1.863 1.807 0 3.28-.806 4.42-1.863 1.085-1 1.808-2.196 2.197-3.03a9.9 9.9 0 0 0-2.197-3.003c-1.14-1.056-2.613-1.89-4.42-1.89"/></svg>'
  },
  'face-smile': {
    value: 'face-smile',
    type: 'Icons',
    description: 'icon.core.face-smile',
    svg: '<svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="M464 256A208 208 0 1 0 48 256a208 208 0 1 0 416 0zM0 256a256 256 0 1 1 512 0A256 256 0 1 1 0 256zm177.6 62.1C192.8 334.5 218.8 352 256 352s63.2-17.5 78.4-33.9c9-9.7 24.2-10.4 33.9-1.4s10.4 24.2 1.4 33.9c-22 23.8-60 49.4-113.6 49.4s-91.7-25.5-113.6-49.4c-9-9.7-8.4-24.9 1.4-33.9s24.9-8.4 33.9 1.4zM144.4 208a32 32 0 1 1 64 0 32 32 0 1 1 -64 0zm192-32a32 32 0 1 1 0 64 32 32 0 1 1 0-64z" fill="var(--sl-icon-fill-default)"></path></svg>'
  },
  fav: {
    value: 'star',
    type: 'Icons',
    description: 'icon.core.fav',
    svg: '<svg viewBox="0 0 576 512" xmlns="http://www.w3.org/2000/svg"><path d="M287.9 0c9.2 0 17.6 5.2 21.6 13.5l68.6 141.3 153.2 22.6c9 1.3 16.5 7.6 19.3 16.3s.5 18.1-5.9 24.5L433.6 328.4l26.2 155.6c1.5 9-2.2 18.1-9.7 23.5s-17.3 6-25.3 1.7l-137-73.2L151 509.1c-8.1 4.3-17.9 3.7-25.3-1.7s-11.2-14.5-9.7-23.5l26.2-155.6L31.1 218.2c-6.5-6.4-8.7-15.9-5.9-24.5s10.3-14.9 19.3-16.3l153.2-22.6L266.3 13.5C270.4 5.2 278.7 0 287.9 0zm0 79L235.4 187.2c-3.5 7.1-10.2 12.1-18.1 13.3L99 217.9 184.9 303c5.5 5.5 8.1 13.3 6.8 21L171.4 443.7l105.2-56.2c7.1-3.8 15.6-3.8 22.6 0l105.2 56.2L384.2 324.1c-1.3-7.7 1.2-15.5 6.8-21l85.9-85.1L358.6 200.5c-7.8-1.2-14.6-6.1-18.1-13.3L287.9 79z" fill="var(--sl-icon-fill-default)"></path></svg>'
  },
  github: {
    svg: '<svg fill="var(--sl-icon-fill-default)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M7.976 0C3.566 0 0 3.667 0 8.203c0 3.626 2.285 6.695 5.454 7.781.396.082.541-.176.541-.393 0-.19-.013-.842-.013-1.521-2.219.489-2.68-.978-2.68-.978-.357-.95-.886-1.195-.886-1.195-.726-.503.053-.503.053-.503.806.055 1.229.842 1.229.842.713 1.25 1.861.897 2.324.68.066-.53.277-.897.502-1.1-1.77-.19-3.632-.897-3.632-4.048 0-.896.317-1.63.819-2.2-.08-.204-.357-1.046.079-2.173 0 0 .673-.217 2.192.842.65-.18 1.32-.27 1.994-.272.674 0 1.36.096 1.994.272 1.519-1.06 2.192-.842 2.192-.842.436 1.127.159 1.97.08 2.173.515.57.818 1.304.818 2.2 0 3.15-1.862 3.843-3.645 4.047.291.258.542.747.542 1.521 0 1.1-.013 1.983-.013 2.255 0 .217.145.475.541.394 3.17-1.087 5.454-4.156 5.454-7.782C15.952 3.667 12.374 0 7.976 0" clip-rule="evenodd"/></svg>'
  },
  hand: {
    svg: '<svg fill="var(--sl-icon-fill-default)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path d="M12.856 2.531c-.201 0-.23.029-.344.029-.287-.802-1.06-1.404-1.948-1.404-.2 0-.373.029-.544.086C9.647.641 8.988.24 8.272.24c-.745 0-1.404.4-1.776 1.002a1.6 1.6 0 0 0-.516-.086A2.07 2.07 0 0 0 3.918 3.22v4.349c-.172-.029-.315-.058-.458-.058-1.26 0-2.292 1.032-2.292 2.32 0 .574.2 1.118.602 1.548l2.32 2.492a5.98 5.98 0 0 0 4.354 1.89h.86c3.036 0 5.528-2.463 5.528-5.5V4.595c0-1.117-.86-2.063-1.977-2.063m.601 7.73a4.126 4.126 0 0 1-4.125 4.125h-.888a4.66 4.66 0 0 1-3.352-1.433L2.83 10.49a.92.92 0 0 1-.286-.66c0-.63.544-.916.888-.916.258 0 .516.115.687.315l.774.802a.22.22 0 0 0 .143.058c.143 0 .23-.086.23-.23V3.22c0-.373.314-.688.687-.688.372 0 .659.315.659.688v4.68c.057.257.257.458.515.458.23 0 .43-.2.43-.43l.029-5.625c0-.372.286-.687.658-.687s.66.315.66.687v5.596c.057.258.257.459.515.459.23 0 .43-.2.43-.43v-4.68c0-.372.315-.687.687-.687s.659.315.659.687v4.68c.057.23.258.43.516.43.229 0 .43-.2.43-.43V4.594c0-.373.286-.688.658-.688s.659.315.659.688z"/></svg>'
  },
  'home-blank': {
    value: 'home-blank',
    type: 'Icons',
    description: 'icon.core.home-blank',
    svg: '<svg viewBox="0 0 576 512" xmlns="http://www.w3.org/2000/svg"><path d="M303.5 5.7c-9-7.6-22.1-7.6-31.1 0l-264 224c-10.1 8.6-11.3 23.7-2.8 33.8s23.7 11.3 33.8 2.8L64 245.5 64 432c0 44.2 35.8 80 80 80l288 0c44.2 0 80-35.8 80-80l0-186.5 24.5 20.8c10.1 8.6 25.3 7.3 33.8-2.8s7.3-25.3-2.8-33.8l-264-224zM464 204.8L464 432c0 17.7-14.3 32-32 32l-288 0c-17.7 0-32-14.3-32-32l0-227.2L288 55.5 464 204.8z" fill="var(--sl-icon-fill-default)"></path></svg>'
  },
  'icon-storybook': {
    svg: '<svg fill="var(--sl-icon-fill-default)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path d="M14.342.754a.806.806 0 0 0-.853-.752l-.882.06.06 1.844c0 .07-.05.12-.12.12-.03 0-.06 0-.08-.03l-.591-.47-.702.53c-.05.04-.13.03-.17-.02a.08.08 0 0 1-.02-.08l.08-1.814-8.811.562a.81.81 0 0 0-.752.832l.491 13.211c.02.421.35.752.772.772L13.5 16h.04c.44 0 .802-.36.802-.802V.754M8.227 12.973c-1.915 0-3.589-.772-3.398-3.468.07-.311 2.506-.241 2.506 0-.03 1.112.22 1.433.862 1.433.491 0 .712-.27.712-.722 0-1.624-4.01-1.674-4.01-4.65 0-1.705 1.173-2.848 3.268-2.848s3.238 1.123 3.238 3.248c-.281.22-2.396.371-2.396.06.04-1.193-.491-1.243-.792-1.243-.3 0-.762.09-.762.722 0 1.574 4.07 1.494 4.07 4.681 0 1.794-1.454 2.787-3.308 2.787z"/></svg>'
  },
  info: {
    svg: '<svg fill="var(--sl-icon-fill-default)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M8 16A8 8 0 1 0 8 0a8 8 0 0 0 0 16M9.125 4.005a1.125 1.125 0 1 1-2.25 0 1.125 1.125 0 0 1 2.25 0M8 6.505a1 1 0 0 1 1 1v4.5a1 1 0 1 1-2 0v-4.5a1 1 0 0 1 1-1" clip-rule="evenodd"/></svg>'
  },
  menu: {
    value: 'bars',
    type: 'Icons',
    description: 'icon.core.menu',
    svg: '<svg viewBox="0 0 448 512" xmlns="http://www.w3.org/2000/svg"><path d="M0 88C0 74.7 10.7 64 24 64l400 0c13.3 0 24 10.7 24 24s-10.7 24-24 24L24 112C10.7 112 0 101.3 0 88zM0 248c0-13.3 10.7-24 24-24l400 0c13.3 0 24 10.7 24 24s-10.7 24-24 24L24 272c-13.3 0-24-10.7-24-24zM448 408c0 13.3-10.7 24-24 24L24 432c-13.3 0-24-10.7-24-24s10.7-24 24-24l400 0c13.3 0 24 10.7 24 24z" fill="var(--sl-icon-fill-default)"></path></svg>'
  },
  'octagon-exclamation-solid': {
    value: 'octagon-exclamation',
    type: 'Icons',
    description: 'icon.core.octagon-exclamation-solid',
    svg: '<svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="M140.6 21.2C154.1 7.7 172.4 .1 191.5 .1l129 0c19.1 0 37.4 7.6 50.9 21.1L490.8 140.6c13.5 13.5 21.1 31.8 21.1 50.9l0 129c0 19.1-7.6 37.4-21.1 50.9L371.4 490.8c-13.5 13.5-31.8 21.1-50.9 21.1l-129 0c-19.1 0-37.4-7.6-50.9-21.1L21.2 371.4C7.7 357.9 .1 339.6 .1 320.5l0-129c0-19.1 7.6-37.4 21.1-50.9L140.6 21.2zM256 128c-13.3 0-24 10.7-24 24l0 112c0 13.3 10.7 24 24 24s24-10.7 24-24l0-112c0-13.3-10.7-24-24-24zm32 224a32 32 0 1 0 -64 0 32 32 0 1 0 64 0z" fill="var(--sl-icon-fill-default)"></path></svg>'
  },
  pinata: {
    value: 'pinata',
    type: 'Icons',
    description: 'icon.core.pinata',
    svg: '<svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="M272 24c0-13.3-10.7-24-24-24s-24 10.7-24 24l0 24L84.4 48C37.8 48 0 85.8 0 132.4c0 36.3 23.3 68.6 57.7 80.1L96 225.3 96 440c0 39.8 32.2 72 72 72l22.4 0c36.2 0 65.6-29.4 65.6-65.6c0-21.2 17.2-38.4 38.4-38.4l19.2 0c21.2 0 38.4 17.2 38.4 38.4c0 36.2 29.4 65.6 65.6 65.6l22.4 0c39.8 0 72-32.2 72-72l0-112c0-57.4-46.6-104-104-104l-136 0 0-200zM84.4 96L224 96l0 125.2-23.3 9.3-19.1-9.6c-8.2-4.1-17.7-4.5-26.2-1.1L144 224.4l0-4.8c0-17.2-11-32.5-27.4-37.9L72.9 167C58 162 48 148.1 48 132.4C48 112.3 64.3 96 84.4 96zM212.6 260.2l17.5-7c7.1 11.3 19.6 18.8 33.9 18.8l144 0c20.2 0 37.8 10.7 47.7 26.7c-6.7-1.8-13.8-1.4-20.3 1.2l-26.7 10.7-19.1-9.6c-8.2-4.1-17.7-4.5-26.2-1.1l-26.7 10.7-19.1-9.6c-8.2-4.1-17.7-4.5-26.2-1.1l-26.7 10.7-19.1-9.6c-8.2-4.1-17.7-4.5-26.2-1.1l-26.7 10.7-19.1-9.6c-8.2-4.1-17.7-4.5-26.2-1.1l-3.4 1.4 0-42.3 23.3-9.3 19.1 9.6c8.2 4.1 17.7 4.5 26.2 1.1zM144 335.6l15.3-6.1 19.1 9.6c8.2 4.1 17.7 4.5 26.2 1.1l26.7-10.7 19.1 9.6c8.2 4.1 17.7 4.5 26.2 1.1l26.7-10.7 19.1 9.6c8.2 4.1 17.7 4.5 26.2 1.1l26.7-10.7 19.1 9.6c8.2 4.1 17.7 4.5 26.2 1.1l26.7-10.7 16.7 8.4 0 54.5-11.4-4.6c-8.5-3.4-18-3-26.2 1.1l-19.1 9.6-32.3-12.9c-15.7-15.8-37.3-25.5-61.3-25.5l-19.2 0c-24 0-45.7 9.8-61.3 25.5l-32.3 12.9-19.1-9.6c-8.2-4.1-17.7-4.5-26.2-1.1L144 392.4l0-56.7zm0 91.2l23.3-9.3 19.1 9.6c7.3 3.6 15.6 4.3 23.3 2.1c-1.1 5.6-1.7 11.3-1.7 17.2c0 9.7-7.9 17.6-17.6 17.6L168 464c-13.3 0-24-10.7-24-24l0-13.2zm296.7-9.3l23.3 9.3 0 13.2c0 13.3-10.7 24-24 24l-22.4 0c-9.7 0-17.6-7.9-17.6-17.6c0-5.9-.6-11.6-1.7-17.2c7.7 2.3 16 1.5 23.3-2.1l19.1-9.6zM176 168a24 24 0 1 0 0-48 24 24 0 1 0 0 48z" fill="var(--sl-icon-fill-default)"></path></svg>'
  },
  'pinata-solid': {
    value: 'pinata',
    type: 'Icons',
    description: 'icon.core.pinata',
    svg: '<svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="M112 318.1l0-63.4 25.8 9.9c14.7 5.6 31.1 4.9 45.1-2.2L259.8 224l28.2 0 144 0c44.2 0 80 35.8 80 80l0 13.2-23.3 9.3-19.1-9.6c-8.2-4.1-17.7-4.5-26.2-1.1l-26.7 10.7-19.1-9.6c-8.2-4.1-17.7-4.5-26.2-1.1l-26.7 10.7-19.1-9.6c-8.2-4.1-17.7-4.5-26.2-1.1l-26.7 10.7-19.1-9.6c-8.2-4.1-17.7-4.5-26.2-1.1l-26.7 10.7-19.1-9.6c-8.2-4.1-17.7-4.5-26.2-1.1l-26.7 10.7L112 318.1zm0 35.8l2.4 1.2c8.2 4.1 17.7 4.5 26.2 1.1l26.7-10.7 19.1 9.6c8.2 4.1 17.7 4.5 26.2 1.1l26.7-10.7 19.1 9.6c8.2 4.1 17.7 4.5 26.2 1.1l26.7-10.7 19.1 9.6c8.2 4.1 17.7 4.5 26.2 1.1l26.7-10.7 19.1 9.6c8.2 4.1 17.7 4.5 26.2 1.1l26.7-10.7 19.1 9.6c8.2 4.1 17.7 4.5 26.2 1.1l11.4-4.6 0 50.5-24 12-17.7-8.8c-9-4.5-19.6-4.5-28.6 0l-16.5 8.3-16.3-10.8c-2.6-1.8-5.7-2.7-8.9-2.7l-176 0c-3.2 0-6.2 .9-8.9 2.7l-16.3 10.8-16.5-8.3c-9-4.5-19.6-4.5-28.6 0L136 414.1l-24-12 0-48.2zm37.3-119.2L34.4 190.6C13.4 179.6 0 157.8 0 133.8L0 128C0 92.7 28.7 64 64 64l112 0 0-32c0-17.7 14.3-32 32-32s32 14.3 32 32l0 166.1-71.4 35.7c-6 3-13.1 3.4-19.3 .9zM112 437.9l9.7 4.8c9 4.5 19.6 4.5 28.6 0l17.7-8.8 16.5 8.3c10.3 5.1 22.5 4.4 32.1-2l7.4-4.9 0 44.8c0 17.7-14.3 32-32 32l-48 0c-17.7 0-32-14.3-32-32l0-42.1zm288-2.7l7.4 4.9c9.6 6.4 21.8 7.1 32.1 2l16.5-8.3 17.7 8.8c9 4.5 19.6 4.5 28.6 0l9.7-4.8 0 42.1c0 17.7-14.3 32-32 32l-48 0c-17.7 0-32-14.3-32-32l0-44.8zM200 144a24 24 0 1 0 -48 0 24 24 0 1 0 48 0z" fill="var(--sl-icon-fill-default)"></path></svg>'
  },
  search: {
    value: 'magnifying-glass',
    type: 'Icons',
    description: 'icon.core.search',
    svg: '<svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="M368 208A160 160 0 1 0 48 208a160 160 0 1 0 320 0zM337.1 371.1C301.7 399.2 256.8 416 208 416C93.1 416 0 322.9 0 208S93.1 0 208 0S416 93.1 416 208c0 48.8-16.8 93.7-44.9 129.1L505 471c9.4 9.4 9.4 24.6 0 33.9s-24.6 9.4-33.9 0L337.1 371.1z" fill="var(--sl-icon-fill-default)"></path></svg>'
  },
  'size=md, Style=Solid': {
    svg: '<svg fill="var(--sl-icon-fill-default)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path d="m4 1.5-.687-.312C3.125 1.094 3 .875 3 .625 3 .281 3.25 0 3.594 0H8c3.313 0 6 2.688 6 6v.469c0 .375-.031.718-.125 1.062L13 12.5H3c0-.625.281-1.187.813-1.562l4-2.72c.343-.218.343-.687.03-.937a.61.61 0 0 0-.718 0L5.594 8.438c-.063.062-.157.093-.219.124l-.125.063c-.469.25-1.031.219-1.5-.094l-.312-.219A1 1 0 0 1 3 7.47V3.344c0-.531.188-1.031.563-1.406zm.75 2.125c0 .375.25.625.625.625C5.719 4.25 6 4 6 3.625A.627.627 0 0 0 5.375 3a.61.61 0 0 0-.625.625M1.688 14.813 3 13.5h10l1.281 1.313a.74.74 0 0 1 .219.5c0 .374-.344.687-.719.687H2.188a.694.694 0 0 1-.688-.687.7.7 0 0 1 .188-.5"/></svg>'
  },
  smile: {
    svg: '<svg fill="var(--sl-icon-fill-default)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path d="M8 10.75c1.06 0 1.805-.487 2.234-.945a.684.684 0 0 1 .974-.058c.287.258.287.688.029.974A4.35 4.35 0 0 1 8 12.125c-1.547 0-2.635-.716-3.266-1.404-.257-.286-.257-.716.029-.974a.684.684 0 0 1 .974.058c.43.458 1.174.945 2.263.945M6.625 6.625a.905.905 0 0 1-.917.917.905.905 0 0 1-.916-.917c0-.487.4-.917.916-.917s.917.43.917.917m2.75 0c0-.487.401-.917.917-.917.515 0 .916.43.916.917a.905.905 0 0 1-.916.917.905.905 0 0 1-.917-.917M15.333 8A7.33 7.33 0 0 1 8 15.333 7.31 7.31 0 0 1 .667 8 7.33 7.33 0 0 1 8 .667c4.04 0 7.333 3.294 7.333 7.333M8 2.042C4.706 2.042 2.042 4.734 2.042 8A5.954 5.954 0 0 0 8 13.958c3.266 0 5.958-2.664 5.958-5.958 0-3.266-2.692-5.958-5.958-5.958"/></svg>'
  },
  sort: {
    svg: '<svg fill="var(--sl-icon-fill-default)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path d="M8.477 15.578c-.282.219-.688.219-.938 0l-5-4c-.344-.25-.375-.719-.125-1.062a.744.744 0 0 1 1.063-.094l4.53 3.625 4.532-3.625a.71.71 0 0 1 1.031.094c.25.343.22.812-.125 1.062l-5 4zm5-11.156a.71.71 0 0 1 .093 1.031.714.714 0 0 1-1.03.125L8.007 1.953 3.477 5.578c-.344.25-.813.219-1.063-.125a.714.714 0 0 1 .125-1.031l5-4c.25-.219.656-.219.938 0z"/></svg>'
  },
  'sort-down': {
    svg: '<svg fill="var(--sl-icon-fill-default)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><g><path d="M7.54 15.578c.25.219.655.219.937 0h-.032l5-4c.344-.25.375-.719.125-1.062a.73.73 0 0 0-.538-.273l.004-.003H2.964l.003.002a.73.73 0 0 0-.553.274c-.25.343-.219.812.125 1.062zM13.57 5.453a.71.71 0 0 0-.093-1.031l-5-4c-.282-.219-.688-.219-.938 0l-5 4a.714.714 0 0 0-.125 1.031c.25.344.719.375 1.063.125l4.53-3.625 4.532 3.625a.714.714 0 0 0 1.031-.125"/></g></svg>'
  },
  'sort-up': {
    svg: '<svg fill="var(--sl-icon-fill-default)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><g><path d="M7.54 15.578c.25.219.655.219.937 0h-.032l5-4c.344-.25.375-.719.125-1.062a.71.71 0 0 0-1.03-.094l-4.532 3.625-4.531-3.625a.744.744 0 0 0-1.063.094c-.25.343-.219.812.125 1.062zM13.57 5.453a.71.71 0 0 0-.093-1.031l-5-4c-.282-.219-.688-.219-.938 0l-5 4a.714.714 0 0 0-.125 1.031.7.7 0 0 0 .64.286l-.012.01h10.072l-.016-.013a.7.7 0 0 0 .472-.283"/></g></svg>'
  },
  star: {
    svg: '<svg fill="var(--sl-icon-fill-default)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M10.493 5.107 8.616.896H7.384l-1.792 4.21-4.567.488-.373 1.174 3.6 3.037-1.193 4.554 1.002.745 3.925-2.442 3.924 2.442 1.003-.745-1.217-4.44 3.652-3.15-.373-1.175zm-2.507-1.63L6.86 6.317l-.444.322-3.1.272 2.488 2.001.172.544-.856 3.039 2.594-1.662h.568l2.595 1.662-.901-2.94.172-.544 2.507-2.1-3-.272-.445-.321z" clip-rule="evenodd"/></svg>'
  },
  storybook: {
    svg: '<svg fill="var(--sl-icon-fill-default)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path d="M14.342.754a.806.806 0 0 0-.853-.752l-.882.06.06 1.844c0 .07-.05.12-.12.12-.03 0-.06 0-.08-.03l-.591-.47-.702.53c-.05.04-.13.03-.17-.02a.08.08 0 0 1-.02-.08l.08-1.814-8.811.562a.81.81 0 0 0-.752.832l.491 13.211c.02.421.35.752.772.772L13.5 16h.04c.44 0 .802-.36.802-.802V.754M8.227 12.973c-1.915 0-3.589-.772-3.398-3.468.07-.311 2.506-.241 2.506 0-.03 1.112.22 1.433.862 1.433.491 0 .712-.27.712-.722 0-1.624-4.01-1.674-4.01-4.65 0-1.705 1.173-2.848 3.268-2.848s3.238 1.123 3.238 3.248c-.281.22-2.396.371-2.396.06.04-1.193-.491-1.243-.792-1.243-.3 0-.762.09-.762.722 0 1.574 4.07 1.494 4.07 4.681 0 1.794-1.454 2.787-3.308 2.787z"/></svg>'
  },
  'svg-sort': {
    svg: '<svg fill="var(--sl-icon-fill-default)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path d="M8.477 15.578c-.282.219-.688.219-.938 0l-5-4c-.344-.25-.375-.719-.125-1.062a.744.744 0 0 1 1.063-.094l4.53 3.625 4.532-3.625a.71.71 0 0 1 1.031.094c.25.343.219.812-.125 1.062l-5 4zm5-11.156a.71.71 0 0 1 .093 1.031.715.715 0 0 1-1.031.125L8.008 1.953 3.477 5.578c-.344.25-.813.219-1.063-.125a.714.714 0 0 1 .125-1.031l5-4c.25-.219.656-.219.938 0z"/></svg>'
  },
  'svg-sort-down': {
    svg: '<svg fill="var(--sl-icon-fill-default)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><g><path d="M7.539 15.578c.25.219.656.219.938 0h-.032l5-4c.344-.25.375-.719.125-1.062a.73.73 0 0 0-.538-.273l.004-.003H2.964l.003.002a.73.73 0 0 0-.553.274c-.25.343-.219.812.125 1.062zM13.57 5.453a.71.71 0 0 0-.094-1.031l-5-4c-.28-.219-.687-.219-.937 0l-5 4a.714.714 0 0 0-.125 1.031c.25.344.719.375 1.063.125l4.53-3.625 4.532 3.625a.715.715 0 0 0 1.031-.125"/></g></svg>'
  },
  'svg-sort-up': {
    svg: '<svg fill="var(--sl-icon-fill-default)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><g><path d="M7.539 15.578c.25.219.656.219.938 0h-.032l5-4c.344-.25.375-.719.125-1.062a.71.71 0 0 0-1.031-.094l-4.531 3.625-4.531-3.625a.744.744 0 0 0-1.063.094c-.25.343-.219.812.125 1.062zM13.57 5.453a.71.71 0 0 0-.094-1.031l-5-4c-.28-.219-.687-.219-.937 0l-5 4a.714.714 0 0 0-.125 1.031.7.7 0 0 0 .64.286l-.012.01h10.072l-.016-.013a.7.7 0 0 0 .472-.283"/></g></svg>'
  },
  'triangle-exclamation-solid': {
    value: 'triangle-exclamation',
    type: 'Icons',
    description: 'icon.core.triangle-exclamation-solid',
    svg: '<svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="M256 32c14.2 0 27.3 7.5 34.5 19.8l216 368c7.3 12.4 7.3 27.7 .2 40.1S486.3 480 472 480L40 480c-14.3 0-27.6-7.7-34.7-20.1s-7-27.8 .2-40.1l216-368C228.7 39.5 241.8 32 256 32zm0 128c-13.3 0-24 10.7-24 24l0 112c0 13.3 10.7 24 24 24s24-10.7 24-24l0-112c0-13.3-10.7-24-24-24zm32 224a32 32 0 1 0 -64 0 32 32 0 1 0 64 0z" fill="var(--sl-icon-fill-default)"></path></svg>'
  },
  xmark: {
    value: 'xmark',
    type: 'Icons',
    description: 'icon.core.xmark',
    svg: '<svg viewBox="0 0 384 512" xmlns="http://www.w3.org/2000/svg"><path d="M345 137c9.4-9.4 9.4-24.6 0-33.9s-24.6-9.4-33.9 0l-119 119L73 103c-9.4-9.4-24.6-9.4-33.9 0s-9.4 24.6 0 33.9l119 119L39 375c-9.4 9.4-9.4 24.6 0 33.9s24.6 9.4 33.9 0l119-119L311 409c9.4 9.4 24.6 9.4 33.9 0s9.4-24.6 0-33.9l-119-119L345 137z" fill="var(--sl-icon-fill-default)"></path></svg>'
  }
};
