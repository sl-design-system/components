...
const generatePublicApis = async (packages, outDir) => {
  for (const [packageName, files] of packages) {
    await writeFile(
      join(outDir, packageName, 'public-api.ts'),
      files
        .map(f => `export * from './${f}';`)
        .join('\n')
        .concat('\n'),
    ...
    await writeFile(
      join(outDir, packageName, 'ng-package.json'),
      `{
  ...
  const ceMap = new Map(
    modules.flatMap(m =>
      m.exports
        ...
        .map(exp => ({
          ...exp,
          package: `@sl-design-system/${m.path?.replace('packages/components/', '').split('/')[0]}`
        }))
...
export default function plugin({ exclude = [], outDir = 'dist' } = {}) {
  return {
