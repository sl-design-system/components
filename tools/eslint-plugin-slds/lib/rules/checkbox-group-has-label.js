...
export const checkboxGroupHasLabel = {
  meta: {
    type: 'problem',
    docs: {
      description:
        'Ensure sl-checkbox-group elements have aria-label/aria-labelledby or are inside a labeled sl-form-field',
      recommended: true,
      url: null
    },
    fixable: null,
    ...
    messages: {
      missingLabel:
        'sl-checkbox-group elements must have aria-label or aria-labelledby, or be inside an sl-form-field with a label'
    }
  },
  create(context) {
    return {
      TaggedTemplateExpression(node) {
        ...
        analyzer.traverse({
          enterElement(element, parent) {
            ...
            if (element.name !== 'sl-checkbox-group') {
              return;
            }
