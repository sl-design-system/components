...
export const checkboxHasLabel = {
  meta: {
    ...
    messages: {
      missingLabel:
        'sl-checkbox elements must have text content, aria-label or aria-labelledby, or be inside an sl-form-field with a label'
    }
  },
  create(context) {
    return {
      TaggedTemplateExpression(node) {
        ...
        analyzer.traverse({
          enterElement(element, parent) {
            ...
            if (element.name !== 'sl-checkbox') {
              return;
            }

            if (
              hasCheckboxLabel(element, analyzer, context.sourceCode) ||
