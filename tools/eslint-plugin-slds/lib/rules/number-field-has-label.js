...
export const numberFieldHasLabel = {
  meta: {
    type: 'problem',
    docs: {
      description:
        'Ensure sl-number-field elements have aria-label/aria-labelledby or are inside a labeled sl-form-field',
      recommended: true,
      url: null
    },
    fixable: null,
    ...
    messages: {
      missingLabel:
        'sl-number-field elements must have aria-label or aria-labelledby, or be inside an sl-form-field with a label'
    }
  },
  create(context) {
    return {
      TaggedTemplateExpression(node) {
        ...
        analyzer.traverse({
          enterElement(element, parent) {
            ...
            if (element.name !== 'sl-number-field') {
              return;
            }
