...
- `exports` / `publishConfig.exports` in `package.json` are **generated by tsdown** on build — copy them from the component you mirror (`exports` → `./src/*.ts`, `publishConfig.exports` → `./dist/*.js`) and don't hand-tune them. Entry points default to `src/index.ts` + `src/register.ts`; extra ones go in a `tsdown: { entry: [...] }` field listing all of them.
