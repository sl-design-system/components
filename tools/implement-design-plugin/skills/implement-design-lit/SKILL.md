...
- A **published package** (`packages/components/<name>/`) has `package.json` in the root and everything else under `src/`: `index.ts` (re-export), `register.ts` (`customElements.define`), `<name>.ts`, `<name>.css`, `<name>.stories.ts`. There is **no per-package `tsconfig.json`/ESLint/stylelint config** — the shared ones in `packages/components/` cover every component. An **example** (folder under the chosen app/example path) is lighter — `.ts` + `.css` + `.stories.ts` co-located, sharing the example package's existing `index.ts`/`tsconfig.json`; no `package.json`/`register.ts`. Copy boilerplate from a neighbour; use `.js` import specifiers for `.ts` modules.
- **No monorepo registration needed** — the root `packages/components/*` workspace glob and tsdown's `workspace: '*'` pick a new package up automatically. Run `yarn install` from the repo root after scaffolding.
- `exports` / `publishConfig.exports` in `package.json` are **generated by tsdown** on build — copy them from the component you mirror (the `@sl-design-system/source` condition in `exports` → `./src/*.ts`, `default` → `./dist/*.js`, and `publishConfig.exports` → `./dist/*.js`) and don't hand-tune them. Local development tools enable the source condition; consumers use `default` without relying on npm to apply `publishConfig.exports`. Entry points default to `src/index.ts` + `src/register.ts`; extra ones go in a `tsdown: { entry: [...] }` field listing all of them.
...
- Plain **`.css`** (not `.scss`), imported via an import attribute: `import styles from './<name>.css' with { type: 'css' };` → `static override styles = styles;`. **No build step, no `.scss.ts`** — the `@sl-design-system/rolldown-plugin-css-sheet` plugin transforms the `{ type: 'css' }` import into a `CSSStyleSheet` (typed by `vite-env.d.ts`) for Storybook/Vite, Vitest and the `tsdown` build alike, and `.css` edits **hot-reload** in the running dev server (no restart).
...
- No style build step, and no build needed while developing — Storybook and the tests consume the TypeScript sources directly. Type-check against the tsconfig that covers your target: `npx tsc -p packages/components/tsconfig.json --noEmit` for an SLDS package, `npx tsc -p examples/lit/tsconfig.json --noEmit` for a Lit example. Both cover every file in that tree, so only fix the errors in **your** files — pre-existing ones elsewhere aren't yours.
- Tests run from the repo root: `yarn vitest --project=unit --run <path>` (drop `--run` for watch mode).
- A full build is `yarn build` (tsdown); components only is `yarn tsdown --config packages/components/tsdown.config.mjs`, with `--filter <package-name>` for a single package.
- Run the dev server with `yarn start`; `.css` and template edits hot-reload — **do not restart Storybook** to pick up changes.
