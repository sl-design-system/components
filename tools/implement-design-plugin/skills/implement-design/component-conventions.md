...
- **Published design-system package** → `packages/components/<name>/` (kebab-case, singular): a full package with `package.json`, everything else under `src/`, Vitest spec, website docs, and a changeset.
- **App / example path** (e.g. `examples/<root>/`): a lightweight, co-located set — the component(s) + a story, nested folders for children, reusing the example package's existing `index.ts`/`tsconfig.json`, and **no** `package.json` / `register.ts` / website docs / changeset (the example is part of an existing private package). Mirror a neighbour under `examples/`.

Hand-authored files for a **published package**:

```
packages/components/<name>/
├── package.json             # the ONLY file in the package root
└── src/
...
Components have **no per-package `tsconfig.json`, ESLint or stylelint config** — `packages/components/tsconfig.json`, `packages/components/eslint.config.mjs` and `packages/components/stylelint.config.mjs` cover every component in the monorepo.

Everything else (`dist/`, `*.d.ts`, `custom-elements.json`, `CHANGELOG.md`) is **generated** — never hand-write it. **Styles are plain `.css` now (not `.scss`) and are imported directly via an import attribute — there is no `.scss.ts`/`build-styles` step.**

### `package.json`

Copy an existing stable component's `package.json` and change only: `name` (`@sl-design-system/<name>`), `version` (`0.0.1`), `status` (`"draft"` for new work), `description`, `repository.directory`, `homepage`. Keep `type`, `customElements`, `exports`, `publishConfig`, `files`, `sideEffects`, and `scripts` byte-for-byte identical. Add only the `lit` dev/peer dependency unless the component genuinely composes another `@sl-design-system/*` package — in which case add it to `dependencies`.

The `exports` and `publishConfig.exports` fields are **generated by tsdown** during the build; copy them from the component you mirror and never hand-tune them afterwards. The `@sl-design-system/source` condition selects TypeScript sources for local development, while `default` points at built JavaScript in `dist`. Local development tools enable the source condition; consumers use `default` without additional configuration. tsdown also generates `publishConfig.exports`, but correct resolution does not depend on npm applying those overrides:

```json
"exports": {
  ".": {
    "@sl-design-system/source": "./src/index.ts",
    "default": "./dist/index.js"
  },
  "./register.js": {
    "@sl-design-system/source": "./src/register.ts",
...
Every package is built with `src/index.ts` and `src/register.ts` as entry points. Only if the component needs **additional** entry points (a separately importable utility module, say) add a `tsdown` field listing all of them — see `shared`, `grid`, `format-date`:
...
Nothing to register. The root `package.json` workspaces include `packages/components/*`, and `packages/components/tsdown.config.mjs` builds every workspace under it (`workspace: '*'`), so a new package is picked up automatically. Run `yarn install` from the repo root after scaffolding so yarn links the new workspace.
...
- **Import styles from the plain `./<name>.css` with an import attribute**: `import styles from './<name>.css' with { type: 'css' };`. This yields a `CSSStyleSheet` (typed via the repo's `*.css` module declaration in `vite-env.d.ts`); assign it directly to `static styles`. There is **no `.scss` and no `build-styles` step** — the `@sl-design-system/rolldown-plugin-css-sheet` plugin transforms the `{ type: 'css' }` import into a constructable stylesheet for Storybook/Vite, Vitest and the `tsdown` build alike, and edits to the `.css` **hot-reload automatically** (no dev-server restart).
...
- **Where they live**: the design tokens resolve in the theme files at `packages/themes/<theme>/theme.css` (every `--sl-*` custom property is declared there). The manifest's Token Reference (built from the design's `get_variable_defs`) is your first source; confirm each name exists by grepping a `theme.css`.
...
- **camelCase suffix gotcha**: token name segments are **camelCase**, not kebab — e.g. `--sl-color-foreground-primary-onBold` (not `-on-bold` or `-on`), `--sl-size-borderRadius-full`, `--sl-fontWeight-700`. When verifying a token exists, grep the **full name followed by `:`** (its declaration) — `grep -F -- '--sl-color-foreground-primary-onBold:' theme.css`. A loose/prefix grep silently "passes" a truncated wrong name (`--sl-color-foreground-primary-on` looks fine but doesn't exist).
...
The class imports `./<name>.css` directly via the `{ type: 'css' }` attribute. There is **nothing to compile** — no `.scss`, no `.scss.ts`, no `build-styles.js`. Storybook/Vite, Vitest and the `tsdown` build all read the `.css` at import time (via `@sl-design-system/rolldown-plugin-css-sheet`), and changes **hot-reload automatically** in the running dev server (no restart needed).
...
Cover: default render, each reflected property (set → `await el.updateComplete` → assert attribute), keyboard interaction, ARIA attributes/role, and form association where applicable. Use the chai-style `expect(el).to.have.attribute(...)` matchers as the existing specs do.

Run from the repo root, not the package — pass a path to scope the run, and drop `--run` for watch mode:

```bash
yarn vitest --project=unit --run packages/components/<name>
```
